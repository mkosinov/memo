...
- **host** — which machine owns the card (single select: `imac` / `macbook` / `hk` / `gcp`). The single ownership source for `In IMPL` / `In Design` cards (2026-09-20, replaced the CLAIM-comment mechanism): stamped automatically on entering those statuses (`status` arg > `GH_BOARD_HOST` env > the container's label file — in-container manager runs resolve themselves), cleared automatically on leaving — except the label survives `PR (G7)` (2026-09-21): a card on CI stays visibly owned by its machine (and returns to it if CI is red) while occupying no IMPL slot — the budget counts only `In IMPL` status; the label clears when the card leaves `PR (G7)`. The auto-impl watcher budgets per host (`HOST_BUDGETS` in the script: imac 2 / macbook 1); an In IMPL card with an empty host blocks no one.
