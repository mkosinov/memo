...
5. **Scout (pre-design recon) — by dispatch, not by hand.** Right after the issue is picked, dispatch built-in read-only subagents to gather facts (zcode — `Explore`, opencode — `explorer`; a cheap model is these agents' default). Default is **one**; wide scope (many subsystems / dependency issues) — several in parallel, one per zone, in a single Agent-tool message. The prompt: issue number, what the issue claims, what to verify against the live tree (dependencies, consumers, patterns) — plus an **actuality check**: is the gap the issue describes still there, claim-by-claim against live code and recently merged PRs.
   ...
   - **Stale-issue auto-close (2026-09-18, user decision):** verdict `stale` — every load-bearing claim of the issue contradicted by the live tree AND the described gap verifiably gone (already implemented or fully superseded; cite `file:line` and the PR/merge that closed it) → comment the evidence on the issue, `gh issue close N --reason "not planned"`, board card → `Not planned`, report in this session. A wrong close is one click to reopen. A doubtful verdict never closes: comment what is off and keep designing. `partially stale` → correct the stale claims in an issue comment and bake the corrections into the concept and spec; the issue body itself is not edited.
...
## 2. Gates (A and C auto by default; B always the user's; board flips strictly at the gate moment)

Restructured 2026-09-18 by user decision: the old interactive G1a brainstorm and the plan-approval G2 are replaced by auto gates; the panel-reviewed spec at Gate B is the main human gate. Board status labels keep the old names (`In Design (G1a)`, `Spec OK (G1b)`, `Ready to IMPL (G2)`) — renaming status options is forbidden for agents (2026-09-09 incident), and the constants are baked into `gh_board.py`.

| Gate | Decides | User stop only when | After the gate |
|---|---|---|---|
| A — concept | what we build / deliberately do NOT (scope boundaries) | ≥2 concepts remain divergent after the filter below | the spec is written, no stop |
| B — spec | the spec — after the panel's consolidated report and fixes | always — the user's OK is mandatory | commit + **push** the spec; board → `Spec OK (G1b)` |
| C — plan | the plan — after plan-reviewer | the plan forces a spec change | auto-OK: fixes folded into the plan text; commit + **push**; board → `Ready to IMPL (G2)`. Gate C is the last point where the discussion may still return to Gate A |
...
5. panel outcome in one line;
6. remaining assumptions as open questions;
7. spec path.
...
Dispatch `plan-reviewer` (verifies the plan faithfully and completely expands the approved spec): the prompt carries the spec path + plan path. No spec-changing findings → Gate C auto-OK: fold the fixes into the plan text, commit + push, board → `Ready to IMPL (G2)` — no user stop. Findings that change the spec (from the reviewer or from writing the plan itself) → STOP: what was found, why the spec changes, the proposed fix — the user decides; this is the last point where the discussion can still return to Gate A.
