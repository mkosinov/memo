...
- The auto-impl watcher sweeps stale cards every loop (`reconcile`): a closed issue sitting in `In IMPL`/`PR (G7)` gets the lost finishing flip (`In-main`/`Not planned` + `merged` line), a run on the watcher's own host whose opencode sessions have been silent over an hour (stuck/dead dispatch — the run CLI is not a liveness marker) returns to `Ready to IMPL` with a BLOCKED auto-log entry (2026-09-22, the third "closed/dead card in In IMPL" incident).
