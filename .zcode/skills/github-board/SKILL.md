...
python3 .opencode/scripts/gh_board.py reconcile [host] [--dry-run] # watcher-side stale-card sweep (top of every auto-impl loop): closed issue in In IMPL/PR (G7) → In-main/Not planned; dead In IMPL run → Ready to IMPL + BLOCKED auto-log entry (host label preserved — sticky progress)
```
