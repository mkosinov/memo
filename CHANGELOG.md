...
### Added
- **GH #284 — Автоподстановка тарифа по возрастной группе посетителя** — branch
  `284-tariff-age-autofill` (9 commits `7e76061a..8d778e0e`, base `d652f84e`; 44 файла,
  +2003/−117; спека `docs/specs/2026-09-19-tariff-age-autofill-284-design.md`, план
  `docs/plans/2026-09-19-tariff-age-autofill-284-plan.md`):
  - **Backend — признак аудитории у тарифа:** новый enum `TariffAudience`
    (`kid`/`adult`/`all`) + колонка `tariffs.audience` (NOT NULL, `server_default="all"`);
    миграция `b3d5f7a9c1e8` добавляет колонку и бэкфиллит по точному названию
    (`lower(trim(title))`: «детский»→`kid`, «взрослый»→`adult`, всё остальное включая
    «единый» и названия с префиксом→`all`; мягко удалённые строки накрыты; downgrade
    дропает колонку — бэкфилл необратим, принято). Pydantic-схемы тарифов типизированы
    enum'ом: неизвестная строка → 422.
  - **Контракты:** `packages/api-client` (`TariffResponse`/`TariffCreate` + fixture
    бэкенд-ответов) и `packages/domain` (`TariffSchema` + transformer'ы) знают поле —
    пометка не теряется на границе API и в админке.
  - **Единый резолвер — один владелец правила дефолтного тарифа:** новый
    `frontend/admin/lib/age-groups.ts` (единственный источник диапазона «Дети» 3–11 —
    из него собираются и опции `AgeSelect`, и классификатор; сентинел «Взрослый» остаётся
    строкой `'adult'`) + `lib/tariff-resolver.ts` `resolveDefaultTariff(tariffs, age)`:
    возраст 3–11 → первый `kid`; остальное (12–17, `'adult'`, пусто) → первый `adult`;
    нет совпадения → первый в списке (легаси-fallback, может пересекать группы); пустой
    список → тарифа нет; `all` в подстановке не участвует никогда; порядок — как отдаёт API.
  - **Все потребители переведены на резолвер:** черновик строки, обработчик смены возраста
    (смена возраста ВСЕГДА переподставляет тариф и цену, затирая ручной выбор — решение
    владельца), `NewRecordTab`, `ClientRecordTab.addAnonymousVisit`, `AddVisitorForm` и
    `defaultTariff` в `useRecordMutations` — дублирующих реализаций дефолта больше нет.
  - **ServiceModal:** в строке тарифа выпадающий «возрастная группа» (детский/взрослый/
    единый, строчными) с программной a11y-меткой по названию тарифа; без валидации
    дубликатов групп.
  - **Сиды + e2e:** тарифы сида получили audience в согласии с названиями; новый
    `e2e/tariff-age-autofill.spec.ts` — восемь сценариев спеки §4 (S1 детская строка,
    S2 взрослая, S3 неполный возраст, S4 затирание ручного выбора и возврат на «Взрослый»,
    S5 «единый», S6 ничего не помечено, S7 настройка групп в редакторе, S8 две детские
    «холсты»); зачистка зацепившихся тестов (ни один не удалён «для зелени»).
  - **Два латентных бага найдены e2e и исправлены (оба spec-required, TDD):** сохранённая
    строка показывала тариф из mount-момента `formState` — добавлена синхронизация
    отображения при переподстановке; черновик, созданный до прихода тарифов, оставался без
    тарифа — добавлен `LateTariffsHealer` (одноразовый догон по null→resolved с резолвом
    по текущему возрасту строки).
  - **Tests:** backend pytest **2331p/0f/15s**; vitest admin **2317/2317**, domain 41/41,
    api-client 395/395, web 347/347; e2e `tariff-age-autofill` **8/8**; визуальный гейт PASS
    (3 скриншота в `/tmp/opencode/284-visual/`); канон `docs/domain-rules/services.md`
...
- **GH #271 — закалка e2e-набора `server-push-invalidation` против CI-флейка С3: окно 10 с,
  ...
  `docs/specs/2026-09-19-e2e-push-invalidation-flake-271-design.md` rev2, план
  `docs/plans/2026-09-19-e2e-push-invalidation-flake-271-plan.md`; 2 файла, +172/−22; только
  тестовый слой, продуктовый код не тронут):
  - **Окно `PUSH_WINDOW` 5с → 10с (`74cf2911`):** `frontend/admin/e2e/fixtures/server-push.ts` —
    одна разделяемая константа покрывает один полный цикл реконнекта SSE (`retry: 5000` +
    рукопожатие) и остаётся втрое ниже app-wide staleTime 30с; инвариант «тест не проходит за
    счёт протухания кэша» сохранён (10с × (1+1 повтор) = 20с < 30с); вместе с константой
    сдвинулись утверждения С1/С4, хелпер тоста обновления и офлайн-спека (всем — больше запаса);
    устаревший комментарий «PUSH_WINDOW = 5s» синхронизирован.
  - **Таймаут набора 120с (`74cf2911`):** `twoPages.setTimeout(120_000)` на блоке спеки
    (test-level override; глобальный `playwright.config.ts` не тронут) — worst-case цепочка С3
    ≈90с, диагностика обязана успеть напечататься до внешнего таймаута.
  - **Inline-повтор push-утверждений С3 (`25e7e3d5`):** бюджет 1 повтор на утверждение (макс.
    2 попытки) — `passiveRearm` (DB-poll и push-1: то же ожидание с новым таймаутом, без
    ре-навигации) и `envelopeRetry` (push-2: конверт «`waitForResponse` GET `/records/view` +
    переход «Записи» + `recordRow not.toBeVisible`» повторяется целиком с повторным переходом;
    `recordRow` отдельного бюджета не получает — арифметика «два конверта по 2×10с» из спеки
    сохранена); кэш-обходящих действий в повторе нет; съеденный повтор печатает `console.warn`
    с именем утверждения — зелёный прогон остаётся интерпретируемым.
  - **Диагностика исчерпания (`25e7e3d5`, `56587530`):** агрегирующая ошибка с номером попытки,
    именем утверждения, обеими ошибками и метаданными `openFrameLogger` — независимый
    ...
    кадров/ответов, URL с query-параметрами, токены; логгер закрывается в `finally` (нет утечки
    соединения между тестами).
  - **Tests:** baseline «до» правки — флейк **НЕ воспроизведён** локально: 3 попытки × 27 тестов
    = **81/81** зелёных, 0 flaky (`/tmp/opencode/271-task1-baseline*.log`; шардовая среда,
    `--repeat-each=3`, проверка осознанно пониженной точности — нет sibling-нагрузки rest-шарда,
    авторитетный гейт CI); после правки — точечно **15/15** и обе push-спеки **27/27 ×3** зелёных,
    0 failed / 0 flaky, предупреждений о съеденных повторах в логе нет; `tsc --noEmit` чисто;
    eslint 0 errors / 36 pre-existing warnings (бюджет 38).
  - **DoD (issue #271 — верификация на PR CI, Task 5):** три подряд полных shard-rest прогона с
    failed=0 **и** flaky=0 + разовая ручная анти-маскировка (route-abort `**/api/v1/events` → С3
    обязан упасть); результаты — в описании PR, в статус-файле не фиксируются до завершения.
  - Status: `docs/status/2026-09-21-e2e-push-invalidation-flake-271.md`
...
### Added
- **GH #263 — Роль мастера v1** — branch `feat/master-role-263` (T0–T10 плана): серверный скоуп «только своё» (активности → записи → визиты/посетители/оплаты/клиенты; фото — своей активности), маска телефона (последние 4 цифры) + email=null в клиент-ответах, полный CUD оплат своих записей и фото своих активностей, создание клиентов без правки чужих (мутации — admin-only), урезанное меню + экран «Нет доступа», шаблон роли от должности (admin>master), контракт скоуп-покрытия. Design spec: `docs/specs/2026-09-10-master-role-design.md`; Plan: `docs/plans/2026-09-10-master-role-263-plan.md`.
- **GH #258, #259 — Расписание: создание занятия кликом по свободному слоту (модалка в режиме `create`) + пустая неделя/день всегда рендерят сетку с подсказкой** — branch `feature/schedule-slot-create-258-259` (10 commits: 71e0672..1ffa309, base ddb2929):
  - **Клик-создание (#258):** клик по пустому слоту (неделя и день — одинаково) открывает `ActivityDetailsModal` в новом режиме `mode='create'` с префиллами из слота (день недели + время начала, редактируемые). Режимы выделены в подкомпоненты (Rules of Hooks — record-хуки в режиме создания гарантированно не выполняются, все шесть точек падения старой заглушки закрыты конструктивно); `TabNav` в режиме создания рисует единственную вкладку-форму «Создание занятия» (заголовок «Новое занятие», `aria-labelledby`), футер с удалением скрыт. Новая форма `CreateActivityTab` (мастер/услуга/локация/день/время/длительность/вместимость/приватное): при выборе услуги подставляется `durationMinutes`, при выборе локации — `defaultCapacity ?? 0` (ровно как у штампа); «Создать» активна только при полном наборе; сохранение через тот же `addActivity` → `POST /api/v1/activities` с дизейблом кнопки и блокировкой закрытия модалки (крестик/Esc/клик мимо) на время запроса; успех → один тост (владелец как у штампа) и карточка в сетке, ошибка → тост ошибки, форма остаётся открытой.
  - **Пустая неделя/день (#259):** ветка-заглушка недели удалена — сетка (заголовки колонок, ось времени, `DayColumn` со слотами, `DndContext`) рендерится всегда, на пустой неделе работают и клик-создание, и штамп (единый код, без отдельных ветвей). Подсказка — ненавязчивый баннер в обычном потоке (без `z-[N]`) с `role="status"`/`aria-live="polite"`, оба текста сохранены («Нет занятий на эту неделю» / «Нет занятий по выбранным фильтрам», единая логика фильтров для недели и дня); в виде дня заголовок-заглушка «Нет занятий на этот день» заменён той же подсказкой.
  - **Регрессионные гарантии (спек §2.3):** вход «+» на карточке (quickAdd), штамп-режим, вкладки существующего занятия, ветки loading/error, линия «сейчас», `__memo-*` бэкдоры — без изменений, покрыты юнит- и e2e-тестами.
  - **Tests:** admin vitest **1808 passed / 0 failed / 0 skipped** (121 файлов; +18 к базовой 1790 — юнит-сьюты режима `create`, `CreateActivityTab`, пустой недели/дня); e2e полный standalone-прогон **364 passed / 13 failed** (все 13 — известное локальное семейство visual-baseline drift в `visual-regression`/`wave6-status-snapshots`, см. tests_workflow «Known caveats», авторитетен CI); новые спеки `schedule-empty-week.spec.ts` (S1–S5: клик-создание на заполненной и пустой неделе, штамп после клика-создания, пустой день, визуальный базлайн модалки в режиме создания) + починенный `admin-clicks-empty-slot` (без `.catch`) — зелёные.
  - **Closes:** #258, #259.
  - Design spec: `docs/specs/2026-09-14-schedule-slot-create-empty-week-design.md` (on branch)
  - Plan: `docs/plans/2026-09-14-schedule-slot-create-empty-week-plan.md` (on branch)
- **GH #262 — Кабинет пользователя: popup-меню, «Мои данные», смена пароля, портрет, персист темы** — branch `feature/user-cabinet-262` (T1–T8 плана; base `7c43b80` (#266)):
  - **Backend — `user_profiles` + `GET/PUT /api/v1/my` (`c10ea19`, `532fd7b`):** приватная половина профиля в отдельной таблице `user_profiles` (1:1 с `users`, ленивое создание строки при первом сохранении); плоский ответ `{role, has_staff, has_master, first/last_name, avatar_url, specialties, patronymic, birth_date, residence_address, birth_place, passport_*, registration_address}`; PUT-семантика «пропущенный ключ = не трогать, явный `null` = очистить»; `specialties` read-only (admin owns мастер-секцию, #266 D5); публичный сериализатор `masters` структурно не содержит приватных полей (D3).
  - **Backend — портрет (`d678a07`):** `POST /api/v1/my/portrait` — multipart-загрузка с precheck `Content-Length` ДО чтения, кап 5 МБ при стриминге, тип по магическим байтам (JPEG/PNG/WebP, имя файла не доверяется), имя хранимого файла = UUIDv4; публичная раздача через Starlette StaticFiles на `/api/v1/files/avatar` (+ `X-Content-Type-Options: nosniff`); успешная загрузка перезаписывает `Staff.avatar_url` и удаляет предыдущий собственный файл (внешние URL не трогаются).
  - **Backend — `change-password` (`31d1776`):** `POST /api/v1/auth/change-password` (под сессией, `current_password` проверяется первым; неверный → 401 `AUTH_INVALID_CREDENTIALS` с timing-parity, политика → 422 `PASSWORD_POLICY`); на успехе текущая сессия остаётся, все остальные сессии пользователя удаляются (D6); лестница блокировок не задействуется; аватар добавлен в снапшот `/auth/me`.
  - **api-client (`a9ce337`):** схемы/эндпоинты `getMyProfile`/`updateMyProfile`/`uploadPortrait`/`changePassword`; `MasterSnapshot.avatar_url`; `PASSWORD_POLICY_HINT_RU`.
  - **Админка — персист темы (`af3a7ae`):** bootstrap-скрипт в `app/layout.tsx` применяет `data-theme` из `localStorage["memo-theme"]` ДО гидрации (тёмная перезагрузка без светлой вспышки); `UIContext.toggleTheme` пишет в storage.
  - **Админка — UserMenu popup (`9b7624e`, `1131128`):** плашка пользователя в сайдбаре — триггер upward-popup ровно из 4 пунктов (слайдер темы, «Мои данные», «Сменить пароль», «Выйти»); старый слайдер-ряд убран из нижней панели; a11y по WAI-ARIA Menu Button (roving tabindex, стрелки/Home/End, Escape, Tab закрывает и возвращает фокус, клик вне закрывает); свёрнутый сайдбар — круглый аватар-триггер; «Аноним» + инициал при отсутствии карточки (D9).
  - **Админка — модалки (`1330441`, `c53a7e0`):** `MyDataModal` (Роль read-only; Имя*/Фамилия* — публичная половина, скрыты целиком при `has_staff=false`; Специализация — read-only строка только при `has_master=true`; приватные поля + секция «Паспорт» с неактивной строкой-заглушкой; портрет-блок: превью + «Загрузить фото»/«Удалить»; dirty-guard на закрытии; submit → инвалидация `['me']` + семейства `staff`) и `PasswordModal` (3 поля + подсказка политики; неверный текущий → inline-ошибка, успех → тост «Пароль изменён» без редиректа).
  - **E2E (`cabinet.spec.ts` + инфра):** сценарии S1–S6 (popup + тема + персист; смена пароля; имя+портрет → плашка и таблица «Сотрудники», специализация read-only; приватные поля персистятся + анонимный `/my` 401 + key-set `/masters` без приватных полей; logout; пользователь без карточки). Инфра: test-scoped `FILES_DIR` (свой каталог аватаров на шард, очистка вместе с БД в `globalSetup` + per-test фикстуре + `e2e-shard-start.sh`), фабрика `seedUser({noStaffId})`, `visual-regression.spec.ts` пинит `localStorage["memo-theme"]="light"`, пере-захват базлайнов `menubar-*`.
  - **Доки:** domain-rules `profile.md` (сущность `user_profiles`, граница public/private, правила файлов аватара), правки `auth.md` (change-password) и `staff.md` (`specialty` CSV admin-owned, `avatar_url`, self-edit через `/my`).
  - **Closes:** #262.
  - Design spec: `docs/specs/2026-09-09-user-cabinet-design.md` (ревизия 3)
  - Plan: `docs/plans/2026-09-11-user-cabinet-262-plan.md`

## [Unreleased] — 2026-09-12

### Added
- **#260 — плашки стопок не перекрывают панели/сайдбар; z-шкала на токенах + блокирующий CI-guard** — branch `260-z-tokens` (7 commits: 8841189..2fe5570, base ddb2929; план T1–T5, 5/5 задач, оба follow-up таска закрыты):
  - **Токены (`8841189`):** `frontend/admin/app/globals.css` — вместо блока-комментария `:root`-блок из 16 токенов (`--z-base` 10 … `--z-toast` 250), единственное место с голыми z-числами; контракт `z-[var(--z-имя)]` в классах и `zIndex: 'var(--z-имя)'` в inline.
  - **Баг (`45077b5`):** плашка стопки в `DayColumn` `z-[110]` → `--z-badge` **26** (выше карточек 25, ниже сайдбара 30 и панелей 50) — больше не перекрывает шапку/поповер и сайдбар; карточная модель `25−z` не тронута (исключение guard'а).
  - **Миграция (`e379cfd`):** ~30 компонентов (schedule, layout, shared-дропдауны, оверлеи/модалки) на z-токены без перенумерации прочих значений; решения архитектора по baseline — девять полноэкранных модалок → `--z-popover`, `DayColumn:174` → `--z-header`.
  - **E2E (`9c7756b`, `ea928b1`):** новый `e2e/schedule-z-layering.spec.ts` S1–S3 (computed-z пары корневого stacking context; S1/S3 были RED на 110), селекторы переведены с z-классов на testid (migration-proof), `wave5-x-cards-blurred` зелёный без правок.
  - **CI-guard (`2fe5570`):** блокирующий `scripts/check_z_tokens.py` в job `frontend-checks` — 127 файлов / 0 нарушений, негативная самопроверка подтвердила exit 1.
  - **Тесты:** admin vitest **1790p/0f** (120 файлов), `tsc` 0, e2e schedule-z-layering 3/3 + wave5-x-cards-blurred 1/1 + records 23/23; две стадии обзоров ✅, финальный full-feature review ✅, visual gate exit 0. Follow-up (вне скоупа): разведение `--z-popover`/`--z-modal` для полноэкранных модалок; преэкзист-флейк `server-push-invalidation.spec.ts:150` (PR #269).
