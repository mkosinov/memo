...
> **GH #243 Unified Delete Contract Phase 1 (record zone: payments + anonymous visits → deferred): ✅ Completed 2026-09-19** — payment delete (client tab of the record modal) and anonymous-visit delete migrated to the shared deferred `PendingActions` pipeline (5s undo window), closing the last instant-delete divergence from the `docs/domain-rules/deletion.md` canon; instant `deletePayment`/`deleteVisit` removed as dead code (S1/S2); undo restores the row at its ORIGINAL position (S5), not appended; honest commit-error toast «Не удалось подтвердить удаление» when the server never responded (S3) implemented in BOTH the `PendingActionsContext` default handler AND `staleAwareOnError` (records/activities surfaces; architect decision per spec «текст применяется ко всем отложенным удалениям»); 404 quiet-success unchanged; tests — vitest **2196p/0f** (139 files), tsc clean, eslint 0 errors/37 warnings (budget 38), e2e S1–S3 records + S5 records/activity, Playwright full **432p/1s/20f** (20 triaged env: 18 font-glyph visual drift + 2 shared-DB seed conflicts — CI authoritative); 17 files, +836/−257 (branch `record-delete-unified-contract-243`, 11 commits `83a4f8db..56743303`, base `8b18b548`; #243 closes on merge; spec rev6 `docs/specs/2026-09-19-record-delete-error-rollback-243-design.md`, plan `docs/plans/2026-09-19-record-delete-unified-contract-243-plan.md`)
