import { getActivities, createBooking } from '../api/activities';
...
describe('getActivities', () => {
  ...
  it('filters by category', async () => {
    const activities = await getActivities({ category: 'взрослым' });
    expect(activities.every((a) => a.category === 'взрослым')).toBe(true);
  });
