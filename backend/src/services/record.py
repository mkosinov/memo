...
- reads: ``RecordService.list`` (plain ORM page) and the Corridor 3 free
  function ``list_records_view`` (display composite — GH #217 Task 1,
  ADR 007: free function in the owning service's module; foreign tables
  are read in ONE query, canon rule 8), ``get`` / ``get_scoped``;
...
The records-list building blocks live at MODULE level (GH #217 Task 1):
``search_fields``, ``_build_list_stmt``, ``_sort_columns`` — shared by
``RecordService.list`` (behavior unchanged) and ``list_records_view``.
...
def map_record(record: Record) -> RecordResponse:
    ...
    imports this, and the ``list_records_view`` free function reuses it
    for the base fields of each view row (``visits`` already loaded via
    selectinload).
    """
...
#
# GH #217 Task 1: module-level shared brick — used by ``_build_list_stmt``
# for BOTH ``RecordService.list`` and the ``list_records_view`` free
# function (the class no longer carries its own copy).
search_fields = [
...
def _build_list_stmt(
    params: RecordListParams, master_key: str | None = None,
) -> Select[tuple[Record]]:
    """Assemble the shared records-list query (GH #213 DRY-glue; module
    level since GH #217 Task 1 — shared by ``RecordService.list`` and the
    ``list_records_view`` free function).

    Activity INNER join + business filters + ``q`` predicates (Client /
    Service LEFT OUTER joins ONLY when ``q`` is present) + eager visits.
    ``list()`` and ``list_records_view()`` share this builder — the view
    is the same query plus extra labeled display columns (§5).
    ...
    if params.q:
        stmt = (
            stmt.outerjoin(Client, Record.client_id == Client.id)
                ...
                .where(search_predicate(params.q, search_fields))
        )
    return stmt
...
class RecordService(GenericService[RecordCreate, RecordUpdate, RecordResponse]):
    ...
    The display composite of the records table lives in the module-level
    ``list_records_view`` free function (Corridor 3 — GH #217 Task 1,
    ADR 007); the class keeps the plain ORM page (``list``) and the
    ...
    async def list(
        ...
        Business filters are hand-written in the module-level
        ``_build_list_stmt`` (G1a principle); pagination/date mechanics
        are shared helpers (BaseRepository.list_entity, day_range).
        ``master_key`` — the per-master scope (GH #263); see the builder.
        """
        stmt = _build_list_stmt(params, master_key=master_key)
        # --- Sort (whitelist map) + Paginate (COUNT before ORDER BY) ---
        items, total = await self._repository.list_entity(
            db_session,
            stmt,
            order_by=_sort_columns(params),
...
async def list_records_view(
    ...
    Corridor 3 free function (canon rule 8 / ADR 007, GH #217 Task 1 —
    behavior-for-behavior move of the former ``RecordService.list_view``
    method; the records table's display composite has no owning
    repository, so it stays a free function in the record service module,
    executed through the repo list mechanics).

    Same query as ``RecordService.list`` — shared module-level
    ``_build_list_stmt`` + shared ``_sort_columns`` whitelist (US-5/US-6
    parity) — plus 8 labeled display columns, riding the owner repo
    ``list_custom`` row-tuple core (ADR 007 execution-path rule:
    "page of rows + total" rides the repository list mechanics).
    ...
    GH #263 T2: ``master_key`` (the requester's scope, passed from the
    route via ``get_scope``) ANDs conjunctively with the client-supplied
    filters in the shared builder — a master sees ONLY his own records;
    ``None`` (admin) adds nothing. Access guards stay in the route.
    """
    ...
    stmt = _build_list_stmt(params, master_key=master_key).add_columns(
    ...
    rows, total = await get_record_repository().list_custom(
        db_session,
        stmt,
        order_by=_sort_columns(params),
        limit=params.per_page,
