...
class TestLocationsCrud:
    ...
    def test_list_locations_includes_created(self) -> None:
        ...
        app = create_app()
        with TestClient(app) as client:
            create_resp = client.post("/api/locations", json=LOCATION_PAYLOAD)
    ...
    def test_update_location(self) -> None:
        ...
        with TestClient(app) as client:
            ...
            response = client.put(f"/api/locations/{location_id}", json=update_data)
            assert response.status_code == 200
            body = response.json()
    ...
    def test_get_nonexistent_location_returns_404(self) -> None:
        ...
        app = create_app()
        with TestClient(app) as client:
            response = client.get("/api/locations/nonexistent-id")
    ...
    def test_delete_nonexistent_location_returns_404(self) -> None:
        ...
        app = create_app()
        with TestClient(app) as client:
            response = client.delete("/api/locations/nonexistent-id")
        assert response.status_code == 404
