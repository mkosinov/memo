...
- **Visits (#297) / payments (#298)** — leaf entities: they carry the **empty** `{expected: {}}`; their routes require the body (bare → 422) and their frontend commits send `{}` — mechanical rewiring lands in #324 (supersedes the "no server changes" conclusion those issues closed with).
...
- **Photo / UserSettings** — leaves: empty `{expected: {}}`, no preview content (nothing to show), ring on delete; wiring lands in #324. **Visitor** — has deps (tags/records), to be wired by #324 under the same full flavor.
...
The `Expected?` column marks the relations whose ids a **deferred** commit carries in `expected` (per-entity id-sets of the confirmed tree); archive-aware entities are never deferred, so their rows are `—`. Deferred entities in this matrix: records, activities, and tags (#318).
