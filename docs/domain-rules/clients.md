...
Client is a soft-delete entity. See `docs/domain-rules/_overview.md` → "is_active semantics on get/update/patch" for the general rule. **Entity note:** `is_active` is now accepted by `ClientUpdate` and `ClientPatch` (defaults flipped to `bool | None = None`, GH #184 §3.5); archive/restore via PATCH is possible. Only the frontend restore-buttons UI remains a follow-up. **#178→#201 window:** `ClientUpdate.is_active` stays optional, but the sticky-preserve injection was removed in #178 — a Client PUT omitting `is_active` errors (500) until #201 redefines Client PUT (explicit-null wipe). The admin Client edit flow (`ClientCardModal` → `ClientsContext`, payload name/phone/email/channel only) sends no `is_active` and is affected in the window.
