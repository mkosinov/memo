...
- **Delete (unified contract, #285 rev7/rev8, GH #139):**
  - **`?dry_run=true` — pure preview** (non-destructive, never touches rows, emits no SSE): zero deps → 204 WITHOUT deleting; deps → 409 + dependency tree; missing record → 404; combined with a resolutions body → 422 `dry_run_with_resolutions_forbidden` (checked before the existence probe).
  - **No-body DELETE abolished (rev7):** bare DELETE (no body, no flag) → 422 `expected_state_required`; a body without `expected` (e.g. `{"resolutions": {...}}` alone — rejected legacy shape) → the same 422. Every real deletion declares its state: clean commit → body `{"expected": {}}` → 204 hard delete; cascade commit → body `{resolutions, expected}` (Visits + Payments + record_tags join rows hard-deleted; Record row physically removed).
  - **`expected` — subset match by id (rev5/rev6):** commit body `{resolutions?, expected}`, where `expected` is per-entity id-sets of the dependencies the user confirmed (from the dry-run/dialog tree's `items`). Server order: existence → collect deps/ids → **expected verification first** → resolutions validation → execution. 409 `stale_dependencies` (+ current tree) when the server holds dependencies **absent from `expected`** — both an appearance and a swap are caught; a dep that disappeared mid-window does not block (subset, not equality). **Auto-deps (`record_tags`, auto=True) are excluded from the match** — they carry no `items` and never appear in `expected` (server verifies only non-auto deps).
  - Deps: visits/payments cascade (auto=False, user must resolve), record_tags cascade (auto=True, resolved server-side).
  - **Contract deviation (deliberate):** records deviate from the shared no-body DELETE pattern — the no-body mode (execute-if-clean 204 / preview 409) remains only on the 5 archive-aware entities (staff/location/service/material/client); for records the only preview is `?dry_run=true`.
- **Delayed delete:** RETURNED (rev7, #285 — user decision 16.09): the 5-second undo window is back, on the **shared PendingActions pipeline** (same as visits/payments: optimistic removal, undo toast «Удалено. Отменить», commit by timer, undo = by-key cache restore) — NOT the old bespoke `setTimeout` mechanism removed by Addendum 13. The commit carries `expected` (id-sets): a mid-window race → 409 `stale_dependencies` → row restored + honest error (see Delete flow below); commit 404 → silent success (record already deleted by a competitor); any other commit failure → row restored + error toast — commit failures are never silent.
- **409 tree `items` (#285 D9б/в):** the record 409 dependency tree (both `has_dependencies` dry-run and `stale_dependencies` commit) carries per-node `items: [{id, label}]` — one one-line entry per dependent row: Visit → `«{service.title}, {price}»` (via `tariff_id`; `tariff_id IS NULL` → `«Без тарифа, {price}»`), Payment → `«{amount}, {method}»` (`method IS NULL` → `«{amount}, —»`), record_tags → `«{tag name}»` (id = tag_id). Other entities keep the bare tree — `items` is OMITTED from their nodes (rev8 serialization `exclude_none=True`: the key is never serialized as `null`). `count`/`cascade_preview` unchanged.
- **409 tree `auto` (rev8):** every node carries `auto: bool` mirroring the FK matrix's auto flag (`record_tags` → `true`; all user-choice deps → `false`, serialized explicitly — `bool` survives `exclude_none`). The client builds the deferred-delete `expected` id-sets by skipping nodes with `auto=true` — by field, not by hardcoding entity names.
...
- **Delete flow (#285, spec D2/D3/D4/D5):** shared unbound `useDeleteRecord` hook (hooks/useDeleteRecord.ts) — both paths deferred, mirroring `deleteVisitDeferred`/`deletePaymentDeferred`:
  - **Clean path `removeRecord(record)`:** dry-run `dryRunDeleteRecord(id)` (`?dry_run=true`) → 204 → item-level snapshot of the row in every `['records', ...]` cache (D5; canonical `qk.record(id)` is not snapshotted) → optimistic removal → `enqueuePendingAction({id: 'delete-record-${id}', message: 'Удалено. Отменить', delayMs: 5000, undo: by-key snapshot restore (no server calls, no invalidations), commit: resolveDeleteRecord(id, {expected: {}})})`.
  - **Cascade path `removeRecordResolved(record, resolutions)`:** DeleteDialog confirm → no second dry-run (tree already in hand); `expected` id-sets built from the tree's `items`, skipping nodes with `auto=true` (by field, not hardcoded names); dialog closes immediately (enqueue is sync); same enqueue with commit `resolveDeleteRecord(id, {resolutions, expected})`.
  - Dry-run 409 → `dependencies` parked → DeleteDialog (entityType "record"), row stays; non-409 dry-run errors (404/500/network) propagate to the call sites' existing error toasts — never intercepted.
  - **Commit errors (D4, in the PendingActions pipeline, `onError`):** 404 → silent success (record already deleted by a competitor — no undo, no toast); 409 `stale_dependencies` (mid-window expected mismatch) → undo + toast «Не удалось удалить: данные изменились» with the «Обновить» action (invalidates `['records']`-family — `staleAwareOnError` in the hook, not in the domain-independent context); any other error → undo + generic error toast. Invalidations run only in commit (non-fatal): `['records']` prefix + `['record', id]` + `['visitors']`. No optimistic removal is ever resurrected after a server-side 204.
...
| DELETE | /api/v1/records/{id} | Commit body `{resolutions?, expected}` — subset expected-match by id first (409 `stale_dependencies` + current tree), then resolutions validation → cascade hard-delete (visits + payments + record_tags cleaned); `?dry_run=true` = pure preview (204 no-deps / 409 tree / 404); bare DELETE (no body, no flag) or body without `expected` → 422 `expected_state_required` (#285 rev7) |
...
**List `?q=` (server `?q=`, GH #212):** the `q` param is declared on `RecordListParams` with `min_length=2` / `max_length=100` via Pydantic `Field`. Substring search over the linked client's `name`/`phone`/`email` and the activity's `service.title`, plus exact `Record.id` equality when `q` parses as a full UUID. Both the `Client` and `Service` joins (LEFT OUTER, since `client_id` is nullable for anonymous records) are added ONLY when `q` is present — the default query plan is unchanged for the no-`q` case. The `q` predicate lands BEFORE the COUNT via `BaseRepository.list_custom`, so `total` reflects the q-filtered set. Searchable fields per the module-level `search_fields` matrix in `backend/src/services/record.py` (module-level shared brick since GH #217 Task 1):
...
**Sort semantics (server-side whitelist map):** the module-level `_sort_columns()` in `backend/src/services/record.py` (module-level shared brick since GH #217 Task 1 — used by both `RecordService.list` and `list_records_view`) builds ORDER BY expressions from a dict keyed by `sort_by`; the key is Literal-validated upstream, so a missing key is impossible. Keys referencing related entities use **correlated scalar subqueries** (mirrors the deleted client-side comparator; note SQLite BINARY collation ≠ JS `localeCompare`):
...
**Implementation:** the `list_records_view()` free function (Corridor 3, GH #217 Task 1 / ADR 007 — behavior-for-behavior move of the former `RecordService.list_view` method; the route calls it directly with the session as an argument, no service dependency) shares the module-level `_build_list_stmt` with `RecordService.list()` (same filters, `q` predicates, `selectinload(Record.visits)`) and adds the display fields as 8 labeled select columns; the row-tuple result rides `BaseRepository.list_custom` over the owner record repository (see Pagination mechanics above). Base fields map via `map_record` on the ORM entity in `row[0]`; `visits` come from that entity (`selectinload` populates it regardless of extra select columns).
