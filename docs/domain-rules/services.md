...
| ~~material_hint~~ | string | ❌ | — | — | null | **Removed by GH #223 (in design)** — replaced by `materials` links; client screen renders `note ?? material.description` |
| tariffs | array | ❌ | — | — | [] | Тарифы (nested) |
| tag_ids | array | ❌ | — | — | [] | IDs тегов |
| materials | array | ❌ | — | — | [] | `[{material_id, note?}]` — GH #223 (in design); create/PUT hard-replace, PATCH absent → preserve; unknown id → 422 |
...
- **Materials (GH #223, in design):** same hard-replace pattern, but each link carries `note: Text NULL`; ids are pre-validated (unknown material_id → 422) — deliberate deviation from tags (which rely on the DB FK violation). `ServiceResponse.materials` = `[{id, title, description, note}]` ordered `title ASC, id ASC`.
...
| PATCH | /api/v1/services/{id} | Partial update (tag_ids hard-replace when sent; materials hard-replace when sent, preserved when absent — GH #223, in design) |
...
- **Paginated `GET /api/v1/services`**: `page` (≥1), `per_page` (1-100, default 20), `status` (active|archived|all, default active), `sort_by` (Literal whitelist: `title, duration, age, tariffs, specialty, archived, created_at`; 422 on unknown; **`material_hint` removed by GH #223 (in design) — known breaking change: old clients sending it now get 422**), `sort_order` (asc|desc, default asc), **`material_id` (UUID, GH #223 in design): filter to services linked to the material; invalid UUID → 422; valid-but-unknown → empty result; `total` reflects the filter**. Default order: `title ASC, id ASC` (spec §4.4). Column mapping: `age`→`min_age`, `tariffs`→count subquery.
...
- Service ↔ Materials — M2M via `service_materials` (per-link `note`, display fallback to material description) — GH #223 (in design)
...
| **service_materials** (join) | NOT NULL PK | **cascade** (auto) | auto — GH #223 (in design); join rows deleted automatically, materials untouched. |
