...
- [ ] **GREEN:** Create `contexts/LocationsContext.tsx` (per Task 7 template, `getLocations`, `defaultSortBy: 'name'`); wrap `app/(main)/locations/page.tsx`; rewrite LocationsTable (same rules: keep search memo over items, drop sort/slice memos, pager from context, `handleSort` → `setSort`, tabs → `setStatus`).
