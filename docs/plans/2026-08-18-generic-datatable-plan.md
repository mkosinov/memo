...
12. **Empty state = "Нет записей" for ALL 8 tables** (DataTable default; wrappers do NOT pass `emptyLabel`). USER RULING 2026-08-20 (mid-IMPL, T6 gate question) explicitly overriding the G1a/G2 "per-entity empty copy preserved, NO unification" clause — for empty states only. Clients' dual empty variants (reset button «Сбросить фильтры» + SVG icon) dropped (reset lives in the filters bar; no functional loss). The `emptyLabel` prop stays in the contract as an optional override. Spec §6.8 policy row + §8 cat 14 amended accordingly. Applied retroactively to T1–T5 in one standalone alignment commit before T6 (wrappers drop `emptyLabel`, unit assertions + visual emptyText map + empty PNG baselines updated); T6–T8 apply it natively (B2 cat 14 flips from "no edits" to "edits expected").
13. **Records delete substrate — USER RULING 2026-08-22 (resolves Addendum 5's STOP-and-report path; the guard fired for real in T8).** T8's guard verified the records 409 dry-run substrate is MISSING: `DELETE /records/{id}` (records.py:158-173) is a plain hard delete — no body, no dry-run (grep `collect_dependencies`/`409` = 0 hits); `FK_MATRIX` (`backend/src/domain/deletion.py`) has NO `Record` root (only Master/Location/Service/Material/Client); `DeleteDialog.tsx:24` has no `'record'` in `DeleteDialogEntityType` and `TITLE_BY_TYPE` is a total map (adding the type without a title = tsc error); api-client exposes only `deleteRecord(id): Promise<void>`; `RecordService.delete` (record.py:148-169) is `@transactional` and cascades visits → payments → record_tags → record SILENTLY today. User ruling: the substrate is NOT dead code (records have real dependents that cascade silently) — build it IN THIS PR (the PR stays single; #139 gains backend work).
    - **Behavior:** (a) dependents (visits, payments, record_tags) are deleted on EXPLICIT user confirmation via DeleteDialog — no blocking, no undo-toast flow; (b) the dialog opens on ANY dependents; a record with none deletes instantly + toast (same 204 path as Materials).
    - **Backend:** add a `Record` root to `FK_MATRIX` with dependents visits/payments/record_tags — the actual cascade stays in `RecordService.delete`; the dry-run/resolution layer PREVIEWS dependents for the 409 and EXECUTES via the existing `@transactional` delete on confirmation. `DELETE /records/{id}` gains body/resolutions support mirroring the masters/clients routes (`resolutions: dict[str, str] | None = Body(...)`). Pytest contract coverage following the other entities' deletion test pattern.
    - **Frontend:** `'record'` added to `DeleteDialogEntityType` + `TITLE_BY_TYPE`; api-client `deleteRecord` gains the body/resolutions analog (exact `deleteClient`/`deleteMaster` pattern); RecordsTable `⋯` «Удалить» → dry-run → dialog (dependents listed) → confirmed delete + toast.
    - **Pre-existing records delete flows aligned (sanctioned B2 edits):** `ClientRecordTab.tsx:137` `window.confirm` → same dry-run dialog path (a bare confirm silently cascades payments — correctness, not just consistency); `ActivityDetailsModal` 5 s deferred-delete + undo toast (`ClientTab.tsx:137-147` / `ActivityDetailsModal.tsx:169-180`) CONVERTS to the dialog — the 5 s deferral was an inline-row affordance with no analog in the dropdown/dialog world (architect decision under the ruling's delegation; one consistent records-delete UX). Their suites' confirm/timer assertions are updated accordingly.
    - **Tests/e2e:** records backend contract tests (new); frontend unit updates for the two converted flows; `records.spec.ts:475` empty-text + `RecordsTable.test.tsx:210` + visual-map records entries per Addendum 12 (native application); dropdown-delete smoke per Task 8 Part C; visual baselines refreshed ONLY where dropdown/menu content changes (locked-delta verification per spec §9).
...
**Part B2 — records delete substrate (Addendum 13, USER RULING 2026-08-22; backend joins the PR):**
- [ ] **Backend** (`backend/`): add a `Record` root to `FK_MATRIX` in `backend/src/domain/deletion.py` with dependents visits/payments/record_tags (PREVIEW-only for the 409 dry-run; execution stays in `RecordService.delete`, record.py:148-169, `@transactional`, cascade order visits → payments → record_tags → record — do NOT duplicate the cascade in the deletion layer). Add body/resolutions support to `DELETE /records/{id}` (records.py:158-173) mirroring the masters/clients routes exactly (`resolutions: dict[str, str] | None = Body(...)`). Pytest contract coverage following the other entities' deletion suites (dry-run 409 with dependents listed, confirmed delete with resolutions, no-dependents instant 204). Worktree venv needs `uv sync --extra dev` before pytest.
- [ ] **api-client:** `deleteRecord` gains the body/resolutions analog — reuse the exact `deleteClient`/`deleteMaster` pattern.
- [ ] **Frontend shared:** `'record'` added to `DeleteDialogEntityType` + `TITLE_BY_TYPE` (DeleteDialog.tsx:24 — total map, both must change together or tsc fails).
- [ ] **Pre-existing flows aligned:** `ClientRecordTab.tsx:137` `window.confirm` → the same dry-run dialog path; `ActivityDetailsModal` 5 s deferred-delete + undo toast (`ClientTab.tsx:137-147` / `ActivityDetailsModal.tsx:169-180`) → converted to the dialog (rationale in Addendum 13). Unit suites for both flows updated (confirm/timer assertions replaced by dialog assertions — sanctioned B2 edits).

**Part C — table migration + NEW action dropdown:**
- [ ] **Guard step (Contract Addendum 5) — RESOLVED by Addendum 13 (USER RULING 2026-08-22):** the records 409 dry-run substrate was verified MISSING; the ruling is to BUILD it in this PR. See Part B2 — the substrate work is part of this task, no longer a blocker.
...
- [ ] **Final-state cleanups (last consumer migrates in this task):** DELETE the ColumnPicker legacy props shim (`storageKey`/`onChange` + internal LS write — Addendum 8 final state; zero remaining consumers after this migration, verify by grep); REMOVE the e2e `.or(...)` legacy `role="button"` fallbacks marked `#139 transition` (Addendum 11 final state — all 8 tables now render `role="menuitem"`).
...
- [ ] Thin `RecordsTable.tsx`: keeps detail panel (~100 ln), `selectedRecord` state, `clientModalId` state + `ClientQuickCard` render; `<DataTable storageKey="records-columns" tableState={useRecords()} onRowClick={(r) => setSelectedRecord(sel => sel?.id === r.id ? null : r)} rowClassName={(r) => selectedRecord?.id === r.id ? 'bg-surface' : undefined} rowKey={(r) => r.id} …>` — NO `emptyLabel` (Addendum 12: unified "Нет записей" default; native application — `records.spec.ts:475`, `RecordsTable.test.tsx:210` and the visual-map records entries updated accordingly). Wrapper larger than others — expected (spec §6.12 caveat).
...
- [ ] `npm run test:all` green (unit + `records.spec.ts` + `unify-caches.spec.ts` guards) AND backend pytest green (incl. new Part B2 contract tests); tsc clean.
- [ ] Visual diff Records × 7 states; expected deltas: dropdown column, skeleton, `↕` already present, empty-state copy «Нет записей» (Addendum 12).
- [ ] Commit(s): backend substrate + dialog/api-client wiring and the table migration may land as separate commits on the branch (single squash PR absorbs them); suggested: `feat(#139): records delete dry-run substrate (T8-B2, Addendum 13)` + `feat(#139): Records migration + ClientQuickCard rename + action dropdown (T8)`.

**DoD:** context aligned (tsc clean across consumers); rename complete (clients-side `ClientCardModal` only remaining); records delete = DeleteDialog + 409 dry-run everywhere (table dropdown, ClientRecordTab, ActivityDetailsModal — no `window.confirm`/5 s deferral left); dropdown e2e smoke green; all records e2e green; backend contract tests green; E2E scenario anchors: spec §5 scenarios 3, 4 verified via `records.spec.ts`.

---

## Final Gate (after Task 8)

- [ ] Full suite: `npm run test:all` from repo root (or per dev-workflow skill) — **backend pytest green incl. the new records dry-run contract tests (Addendum 13 — backend joined the PR)**, admin vitest + e2e green, tsc clean, build clean. Prune the env to ONE canonical dev stack (:8000 backend / :3001 frontend) BEFORE the run — multi-stack contamination + stale `.next` bundles were the recurring flake source (T6 root cause).
