...
- **Activity modal — statistics row becomes uniform for all clients** (disclosed plan-review delta, refines spec §7 "no UI changes"): the «Статистика» block in record tabs previously showed server stats ONLY for first-20 clients (paged map) and «—» for everyone else; now it derives aggregates (records count / last record / missed / paid) over the same 100-record window the ClientQuickCard uses. First-20 clients: near-identical in practice; clients with >100 records: modal stats now reflect the 100-record window instead of server-wide totals (a known, accepted divergence); everyone gets real numbers instead of «—». No markup/copy changes.
...
### DoD
- New hooks exist with qk keys + 1h dict staleTime; hooks layer fully qk-sourced (verify: `grep -rnE "queryKey: ?\['" frontend/admin/hooks frontend/admin/lib/cache` → ZERO hits — all key literals now live only in `lib/queryKeys.ts`). Hook suite green; tsc/lint clean.
...
- `ClientTab.tsx`: resolve its client via `useClient(record.client_id)` (progressive: «…»/«Без контакта» states, same as ClientLabelById incl. the `!client_id` → «Без контакта» rule); its activity query (:72-76) → `useActivity(record?.activity_id)`. The `client` PROP from the modal's map is removed — ClientTab owns resolution.
  **Stats re-source (plan-review r1-f2 + r2-f3 + r3-f2 decision):** `useClient` returns `ClientResponse` (no `records_count/missed_records/last_record/total_paid`), and ClientStatistics renders an all-«—» grid when `stats` is undefined — a visible regression for first-20 clients, and per-record fabrication is forbidden. DECISION — mirror the ClientQuickCard data recipe (the in-repo precedent for client aggregates): ClientTab loads `useClientRecords(clientId)` (Task 1 hook) and derives: `recordsCount = clientRecords.length`; `lastRecord` = max over the records' ACTIVITIES (`useActivitiesForRecords(clientRecords.map(r => r.activity_id))` — `RecordResponse` has no start time; start lives on `ActivityResponse.start`); `totalPaid` via `usePaymentTotals(recordIds)` (same hook ClientQuickCard uses); `missedRecords` from the visit-status counts across those records (aggregate `visits` by the same missed-status rule the tab already uses). Aggregates are over the SAME 100-record window ClientQuickCard shows — consistency with the existing card outweighs parity with server-wide totals (server stats span ALL records; a >100-record client's modal numbers will differ from today's — DISCLOSED in Behavioral Delta). NEVER fabricate a per-record value as a client aggregate; a field genuinely underivable → omit («—»).
...
**6.2b** Stale-comment sweep (plan-review r3-f4): `app/(main)/schedule/page.tsx:31-33` comment "re-homed to useClients() + per-id getClientById" is now outdated — update it to describe per-tab `useClient` resolution. Also `grep -rn "useClients(" frontend/admin/app frontend/admin/contexts frontend/admin/components` → only legitimate `useClientsTable` matches remain.
...
### DoD
- `grep -rnE "useQuery[<(]" frontend/admin/app | grep -v __tests__` → ZERO hits (all components now clean — the DoD gate 3, same pattern as Task 10.1; catches generic `useQuery<T>(` forms).
...
**11.3** RED-GREEN e2e **US-6** (add to `clients.spec.ts`), WITH cache priming (plan-review r3: without it the test is vacuous — post-migration /schedule holds no clients cache, so /clients always fetches fresh): (1) SPA-navigate to /clients (sidebar click — primes `['clients',...]` cache), wait for table; (2) SPA-navigate to /schedule; (3) quick-add a booking with a NEW client (name+phone; follow existing quick-add patterns in records.spec.ts / schedule specs); (4) SPA-navigate back to /clients; (5) assert the new client's name is visible WITHOUT reload. Discrimination: the primed list is ≤30s stale (staleTime 30_000, no refetch on remount) → WITHOUT the Task 9 invalidation the cached page lacks the new client → test fails; WITH it → refetch → visible. The 9.1 unit pin remains the primary invalidation gate.
