...
6. Run `npm test` + `npm run type-check` in `packages/domain` — expect PASS there but **tsc in frontend/admin will now fail** (expected: transformers/views still use old types — do NOT fix here; Task 3+ consumes). Verify only that domain package itself is green: `cd packages/domain && npx tsc --noEmit`.
...
4. `cd packages/api-client && npm test && npx tsc --noEmit` → green (243+).
...
4. `lib/utils.ts`: delete `decimalToHHMM`/`hhmmToDecimal` (:76-86), `formatTime` (:154-158), `getMonday` (:161-170), `formatDateISO` (:197-202), `generateTimeSlots` (:214-221), `calculateGridTimeRange` (:240-276 incl. `GridActivity`/`GridTimeRange` interfaces). KEEP `HOURS_START`/`HOURS_END`/`SLOT_COUNT` constants and display formatters (`formatDate`, `formatDateRu`, `formatActivityContext`, `formatActivityLabel`, `formatActivityDateTime`, `formatRecordLabel`, `displayMasterName`, color utils, DAYS/MONTHS). Where a kept display helper needs a time primitive (e.g. `formatActivityDateTime` uses local getters — fine as-is), leave it self-contained; NO re-exports of moved helpers (spec §7 import policy).
   - NOTE: deleting utils.formatTime/getMonday WILL break tsc for their current importers (Menubar, Topbar, schedule components, records) — that's the next tasks' work. To keep this task independently green: in THIS task, migrate the trivial display-only importers that don't need other changes: `Menubar.tsx:10`, `Topbar.tsx:5`, `BookingFilters.tsx:6` (they use `getMonday`/`formatDateISO`→`toISODate`/`formatTime` with minute args already? — VERIFY: if they pass float hours, leave them for their component tasks and instead temporarily keep `formatTime` in utils until Task 8/9; the guiding rule: this task's commit must leave `npm run type-check` green or the breakage explicitly delegated to the immediately following task; preferred: migrate Menubar/Topbar/BookingFilters only if their usage is arg-compatible (minutes/display-only), else defer entire file to its scheduled task).
   - Simplest safe order (RECOMMENDED): create datetime.ts + tests (green standalone), commit; delete utils functions in Task 10 (sweep) AFTER all importers migrated. Adjust: in this task ONLY add the module + tests; the utils deletions happen in Task 10.
...
5. `npm test` (expect only tests fixed in this task to run green; ScheduleContext/views still compile? — NO: ScheduleContext imports useActivities→transformActivity and services use old Service type. **This task breaks admin tsc by design.** To keep tasks bite-sized but green, this task ALSO does the minimal compat moves: delete `hooks/useActivities.ts` + its dead import in ScheduleContext.tsx:7 (spec §8 — one-line removal), and delete `__tests__/useReactQueryHooks.test.tsx` useActivities suite (:220-260 + mock :22). The rest of ScheduleContext still references `Activity`/hours — migrated in Task 6. If tsc still fails on ScheduleContext after the useActivities removal, that's acceptable residue for Task 6 as long as `npm test` passes for the suites touched here (vitest doesn't typecheck the whole app). State in commit message: "intermediate: ScheduleContext migrated next".
   - Actual rule: this task's DoD = vitest green for transformers/buildSchedule/buildAdminSchedule suites + `npx tsc --noEmit` errors confined to files scheduled in Tasks 5-9 (list them explicitly in the report).
6. Commit: `feat(admin): transformers slim to master/service/location, buildAdminSchedule startMinutes (#142)`
...
1. Delete from utils.ts: `decimalToHHMM`, `hhmmToDecimal`, `formatTime`, `getMonday`, `formatDateISO`, `generateTimeSlots`, `calculateGridTimeRange` (+ GridActivity/GridTimeRange). All importers were migrated in Tasks 3(deferred)/5-9; run gates to catch stragglers (e.g. `Menubar.tsx`/`Topbar.tsx` used `getMonday`/`formatDateISO` — migrate their imports to `@/lib/datetime` equivalents now if not already done; arg-compat: getMonday identical, formatDateISO→toISODate rename).
2. Run the GATES (spec §9, all from `frontend/admin/` or repo root) and fix any residue:
