...
**Steps:**
- [ ] `cd frontend/admin && git mv "app/(main)/records/components/BookingFilters.tsx" "app/(main)/records/components/RecordsFilters.tsx"`
- [ ] `git mv __tests__/BookingFilters.test.tsx __tests__/RecordsFilters.test.tsx`
- [ ] В `RecordsFilters.tsx`: интерфейс `BookingFiltersProps` → `RecordsFiltersProps` (:14), компонент `BookingFilters` → `RecordsFilters` (:37 и использование :49), `testIdPrefix="booking-filters-status"` → `"records-filters-status"` (:205)
- [ ] В `records/page.tsx`: импорт (:5) и JSX (:23) → `RecordsFilters`
- [ ] В `RecordsFilters.test.tsx`: коммент (:2), импорт (:34), все обращения (:45,47,71,75,125,152,157,226,301) → `RecordsFilters`
- [ ] `npm run test -- RecordsFilters` — ожидание: все тесты файла passed
- [ ] `npm run type-check` — ожидание: ошибок нет
- [ ] Commit: `git commit -am "refactor(#103): rename BookingFilters to RecordsFilters (component, props, testid, unit test, page.tsx consumer)"`
...
**Steps:**
- [ ] Во всех перечисленных e2e-файлах заменить селектор `booking-filters-status` → `records-filters-status`: `records.spec.ts` — 14 строк (96,112,142,143,148,170,171,204,205,239,572,573,728,729); `wave6-status-shared.spec.ts:27,28`; `visual-regression.spec.ts:65,68`
- [ ] Комменты-ссылки на старое имя → `RecordsFilters`: `wave6-status-shared.spec.ts:9,12`; `records-view.spec.ts:35,193`; `fixtures/helpers.ts:348`; `PhotosFilters.tsx:12,186`
- [ ] `grep -rn "booking-filters-status" e2e app` — ожидание: пусто
- [ ] `npm run test:e2e -- records.spec.ts` — ожидание: passed (все тесты файла; оба шарда, где файл заявлен)
- [ ] `npm run test:e2e -- wave6-status-shared.spec.ts` — ожидание: passed
- [ ] Commit: `git commit -am "refactor(#103): records filter e2e selectors to records-filters-status"`
...
**Steps:**
- [ ] `cd frontend/admin && git mv app/components/modal/ActivityDetailsModal/NewBookingTab.tsx app/components/modal/ActivityDetailsModal/NewRecordTab.tsx`
- [ ] В `NewRecordTab.tsx`: тип `NewBookingSubmitData` → `NewRecordSubmitData` (:20, алиас остаётся `= CreateRecordInput`), интерфейс `NewBookingTabProps` → `NewRecordTabProps` (:22,25), компонент `NewBookingTab` → `NewRecordTab` (:29), testid `new-booking-tab` → `new-record-tab` (:134), HTML id + htmlFor: `booking-name`→`record-name` (:147,151), `booking-seats`→`record-seats` (:164,168), `booking-channel`→`record-channel` (:232,236)
- [ ] `index.ts:5`: экспорт → `NewRecordTab`
- [ ] `ActivityDetailsModal.tsx`: импорт (:12, включая `type NewBookingSubmitData` → `NewRecordSubmitData`), tab id `'new-booking'` → `'new-record'` (:131, :221, :252), хендлер `handleNewBookingSubmit` → `handleNewRecordSubmit` + коммент «New booking submit handler» (~:224,229-230,257), рендер (:254), коммент «activity bookings need…» → «activity records need…» (:149)
- [ ] `mockData.ts:172`: коммент «Tariffs (used by SettingsTab, NewBookingTab, ClientTab)» → «…NewRecordTab…»
- [ ] `grep -rn "NewBookingTab\|new-booking" app/` — ожидание: пусто
- [ ] `npm run type-check` — ожидание: ошибок нет
- [ ] Commit: `git commit -am "refactor(#103): rename NewBookingTab to NewRecordTab (tab id, testid, field ids, handler)"`
...
**Steps:**
- [ ] Импорт (:10) → `NewRecordTab`; section-комменты :411, :977, :1029; describes «NewBookingTab…» → «NewRecordTab…» (:413, :979, :1034, :1160); все `render(<NewBookingTab …>)` — 17 вызовов (:422,427,432,437,442,988,994,999,1005,1010,1022,1054,1117,1138,1179,1196,1208); коммент «Serve the activity's booking records» → «…the activity's records» (:175)
- [ ] `grep -rn "NewBookingTab" __tests__/` — ожидание: пусто
- [ ] `npm run test -- ActivityDetailsModal` — ожидание: passed
- [ ] Commit: `git commit -am "refactor(#103): rename NewBookingTab suite in vitest to NewRecordTab"`
...
**Steps:**
- [ ] Селектор `new-booking-tab` → `new-record-tab`: `activity-details-modal.spec.ts:61,425`; `error-messages.spec.ts:91`; `fixtures/helpers.ts:300`
- [ ] `server-push.ts:260`: «The booking lands on…» → «The record lands on…» (созданная запись прилетает по SSE). `server-push.ts:259`, `error-messages.spec.ts:90` — НЕ трогать (процессные, спека §4.4)
- [ ] `grep -rn "new-booking-tab" e2e/` — ожидание: пусто
- [ ] `npm run test:e2e -- activity-details-modal.spec.ts` — ожидание: passed
- [ ] `npm run test:e2e -- error-messages.spec.ts` — ожидание: passed
- [ ] Commit: `git commit -am "refactor(#103): modal tab e2e selectors to new-record-tab"`
...
**Steps:**
- [ ] `visual-regression.spec.ts:118`: заголовок теста «activity modal — new booking tab» → «activity modal — new record tab»
- [ ] `visual-regression.spec.ts:127`: аргумент `toHaveScreenshot('modal-new-booking.png')` → `'modal-new-record.png'`
- [ ] `ls e2e/visual-regression.spec.ts-snapshots/modal-new-booking-*` — ожидание: 2 файла (`-chromium-linux.png` и `-shard-rest-linux.png`)
- [ ] `git mv` каждого: суффиксы сохранить, имя `modal-new-booking` → `modal-new-record` (например `git mv "e2e/visual-regression.spec.ts-snapshots/modal-new-booking-chromium-linux.png" "e2e/visual-regression.spec.ts-snapshots/modal-new-record-chromium-linux.png"` и второй файл аналогично)
- [ ] `npm run test:e2e -- visual-regression.spec.ts` — ожидание: passed БЕЗ флага `--update-snapshots` (доказательство пиксельной идентичности: UI не менялся, менялись только имена)
- [ ] Если шаг выше падает по diff — это сигнал, что UI изменился не от переименования: разбирать по `npm run test:e2e:update -- visual-regression.spec.ts` + ручной сверке diff-картинки, НЕ коммитить вслепую
- [ ] Commit: `git commit -am "refactor(#103): rename modal-new-booking visual baseline to modal-new-record"`
...
**Steps:**
- [ ] `grep -rni "booking" frontend/admin packages/domain packages/api-client` — каждая находка сверяется со спекой §4.4 + D4/D5 (процессные комменты, BookingVisitor/BookingStep, «Booking Context Types»); «booking» в значении данных = 0
- [ ] `grep -rni "booking" docs/domain-rules docs/business-logic.md` — только whitelist спеки §8 (9 строк + `business-logic.md:49`)
- [ ] `npm run test` — ожидание: весь vitest passed
- [ ] `npm run test:e2e -- records.spec.ts activity-details-modal.spec.ts error-messages.spec.ts wave6-status-shared.spec.ts visual-regression.spec.ts` — ожидание: passed
- [ ] `npm run type-check && npm run lint` — ожидание: чисто
