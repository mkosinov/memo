...
**Спека:** `docs/specs/2026-09-16-deferred-record-deletion-design.md` (rev9 — единый контракт отложенного удаления: ветвь `stale_dependencies` получает честную ошибку с действием «Обновить», auto-зависимости исключены из сверки, subset-семантика, критерий = серверные зависимости; занятия #286 имеют зависимости — их контракт ведёт спека #286; визиты/оплаты — листовые follow-up #297/#298, НЕ в скоупе этого плана).

**Хард-гейта нет.** Соседние дорожки: #94 (смержен `8520609` 17.09 — кольцо уже в main), #286 (занятия — свой домен, depends-on этот спека-контракт). Перед каждым коммитом — `git pull --rebase origin main`.
...
- **Сценарий S6 (гонка середины окна):** пока тикает окно, коллега добавляет в запись визит/платёж — в момент commit сервер видит, что зависимостей больше, чем я подтверждал (сверка по id), отдаёт ошибку: строка возвращается, **новая зависимость не удаляется**, красный тост «Не удалось удалить: данные изменились» с действием «Обновить» (rev8) — клик перечитывает списки записей.
...
- [ ] Голая ветка (без флага, без тела) — **упраздняется (rev7)**: → `422 {"detail": "expected_state_required"}` (шаг (д) ниже); ветка execute-if-clean не существует.
- [ ] Docstring роута переименовать: «dry-run (no body)» → «no body: forbidden — 422 expected_state_required (rev7); ?dry_run=true = pure preview»; флаг описать как pure preview.
...
### Steps
- [ ] Тело DELETE записей заменить с `resolutions: dict | None = Body(embed=True)` на **два плоских параметра** (не pydantic-модель — панель wave 2, феасабилити: `Body(embed=True)` на модели форсирует вложение `{"record_delete_body": {...}}` и плоский `{resolutions, expected}` парсился бы как `expected is None` → 422 на каждом execute): `resolutions: dict[str, str] | None = Body(default=None, embed=True)`, `expected: dict[str, list[int]] | None = Body(default=None, embed=True)` (тот же паттерн, что у шести соседних эндпоинтов — `records.py:239`, `clients.py:191`, …). **Ревизия rev7: на execute-пути (не dry_run) `expected` обязателен — в обработчике: оба параметра None → `422` `{"detail": "expected_state_required"}`**.
- [ ] В `deletion.py` — **один проход сбора с id** (панель wave 2, простота: не плодить параллельный коллектор рядом со счётчиковым): дополнить узлы зависимостей записей id-списками (`items` — Task 3) и сверять `expected` по id из `items` не-авто узлов; авто-зависимости (record_tags) в expected-сверке **не участвуют** — отсечение по явному полю `auto` узла (Task 3), не по имени сущности.
...
### Steps
- [ ] `DependencyNode` (deletion.py:235-246) дополняется `items: list[DependencyItem] | None = None`, где `DependencyItem {id: int, label: str}`, и **`auto: bool = False`** (панель wave 2, феасабилити: явная метка авто-зависимости — клиент отсекает по полю, не по хардкоду имён; record_tags узел получает `auto=true`). Поля `count` и `cascade_preview` не удаляются. Сериализация дерева — `exclude_none=True`, чтобы `items: null` не поехал в 409-пейлоадах других сущностей (панель wave 2).
...
- [ ] `useDeleteRecord.ts` — rewrite (имя хука прежнее; мутации `deleteMutation`/`resolveDelete` удаляются):
  - `removeRecord(record: RecordView): Promise<void>`: `snapshots = captureRecordSnapshots(...)` → `await dryRunDeleteRecord(id)` (ошибки — throw наверх, НИКАКОГО перехвата в хуке) → `removeRecordRow(queryClient, snapshots)` → `enqueuePendingAction({ id: 'delete-record-${record.id}', kind: 'delete', message: 'Удалено. Отменить', delayMs: 5000, undo: () => restoreRecordSnapshots(queryClient, snapshots), onError: staleAwareOnError(queryClient), commit: async () => { await resolveDeleteRecord(record.id, { expected: {} }); removeRecordRow(queryClient, snapshots); try { await invalidateAfterDelete(record.id); } catch { /* eventual consistency: SSE/следующая загрузка */ } } })`, где **`staleAwareOnError` (rev8, ветвь честной ошибки в ПОТРЕБИТЕЛЕ записей, не в общем контексте — панель wave 2, простота): `err.status === 409 && err.dependencies` → `action.undo()` + `showToast('Не удалось удалить: данные изменились', 'error', { action: { label: 'Обновить', onAction: () => invalidateEntities(queryClient, ['records']) } })`** (`invalidateEntities` требует `QueryClient` первым аргументом — `lib/invalidate.ts:130`; хук уже держит `queryClient`; канонический `qk.record(id)` не покрыт — модалка к commit закрыта, D5); прочие не-404 — дефолт контекста.
  - `removeRecordResolved(record: RecordView, resolutions: Record<string, string>, dependencies: DependencyNode[]): Promise<void>`: `snapshots = captureRecordSnapshots(...)` → optimistic `removeRecordRow` → `enqueuePendingAction({ id: 'delete-record-${record.id}', ..., onError: staleAwareOnError(queryClient), commit: async () => { await resolveDeleteRecord(record.id, { resolutions, expected: expectedFromDependencies(dependencies) }); ... как выше } })`, где `expectedFromDependencies` = для каждого узла с `items` И `auto === false` → `{[entity]: items.map(i => i.id)}`.
...
- [ ] Тесты `__tests__/useDeleteRecord.test.ts` — rewrite: (а) 409 → прокидывается наверх, enqueue НЕ вызван, кэш не тронут; (б) 204 → enqueue с id `'delete-record-${id}'`, message «Удалено. Отменить», delayMs 5000; (в) commit вызывает `resolveDeleteRecord` с `{expected: {}}`, потом remove + invalidate; (г) провал `resolveDeleteRecord` в commit → onError-путь (покрывается Task 6 — здесь только ассерт, что throw не пойман в хуке); (д) `removeRecordResolved` строит `expected` из `dependencies` (items → id-массивы; узлы с `auto === true` пропускаются — по полю, панель wave 2); (е) undo возвращает строки в исходные кэши (2 кэша: конверт + плоский список — restore по-ключам, форма сохранена); (ж) **`staleAwareOnError`: commit throw 409 + dependencies → undo вызван + `showToast('Не удалось удалить: данные изменились', 'error', {action: {label: 'Обновить', ...}})`; клик по action вызывает инвалидацию `['records']`-family (мок invalidate); не-409-ошибка идёт в дефолт контекста** (панель wave 2).

### DoD
- Vitest админки зелёный; хук не делает серверных вызовов до commit (кроме dry-run); undo чистый (восстановление по-ключам); stale-ветвь («Обновить») в onError хука — юниты (ж) зелёные; `tsc --noEmit` чист.
...
- [ ] Таймер commit (`:64-68`): тело обернуть — `try { await action.commit(); } catch (err) { if (action.onError) action.onError(err); else if (err instanceof ApiError && err.status === 404) { /* тихий успех: цель достигнута */ } else { action.undo(); showToast('Не удалось удалить. Изменение отменено', 'error'); } }`. Импорт `ApiError` — тот же, что в хуках (греп импорта в `useRecordMutations.ts`). **Ветвь `stale_dependencies` НЕ в общем контексте** (панель wave 2, простота: контекст домен-независим; `invalidateEntities(['records'])` — знание records) — она в `onError` потребителя записей (Task 5, `staleAwareOnError`); дискриминатор `err.status === 409 && err.dependencies` — без расширения `ApiError` (409-with-deps в commit бывает только от expected-сверки: `has_dependencies` отвечает лишь no-body/dry-run пути).
- [ ] **Действие-слот тоста — АДДИТИВНО (панель wave 2, простота: миграция undo — churn на только-что смерженном слое #94/#261):** `Toast` сохраняет `undo?: () => void`; добавляется опциональный `action?: { label: string; onAction: () => void }` (3-й аргумент `showToast(message, kind, { action, ... })`); кнопка `ToastContainer.tsx:50-59` рендерит `action.label ?? 'Отменить'`, гейт — `toast.undo || toast.action`; lifetime-логика «undoFn → countdownMs ?? 5000» НЕ трогается (undo-тосты остаются на `undo` — S2/S4 безопасны, панель wave 2, феасабилити: перенос undo на action опустил бы undo-тост до 4500мс при окне 5000мс). Существующие ассерты «Отменить» в `ToastContainer.test.tsx:51-139` не меняются.
- [ ] **Undo-гейт при in-flight commit (панель wave 2, феасабилити):** таймер-запись снимается ДО `await action.commit()` — клик по «Отменить» во время незавершённого DELETE не может локально воскресить строку, которую сервер уже удалил (§8 DoD «undo не воскрешает сервер-удалённое» — в механике, не только в тексте).
...
- [ ] Тесты `PendingActionsContext.test.tsx`: (а) commit resolves — ничего; (б) commit throw 404 (`ApiError`) — undo НЕ вызван, error-тоста нет; (в) commit throw прочее — `undo()` вызван, `showToast('Не удалось удалить. Изменение отменено', 'error')` вызван; (г) action с кастомным `onError` — вызван с ошибкой, дефолт (undo/тост) НЕ сработал; (д) commit визита/оплаты без onError — дефолтный путь (существующие таймерные тесты `:69-237` не меняются); (е) **undo-гейт при in-flight commit: клик «Отменить» во время незавершённого commit не вызывает undo после его успеха** (панель wave 2); (ж) action-слот: тост с `action.label` рендерит кастомный текст, с `undo` — «Отменить» (регресс).

### DoD
- Vitest зелёный; дефолт onError: 404 — тихо, прочее — undo + error-тост; `stale_dependencies`-ветвь — в потребителе записей (Task 5), не в контексте; кастомный onError перекрывает; action-слот аддитивен (`action {label, onAction}` рядом с `undo`), lifetime undo-тостов не тронут; undo гейтится при in-flight commit; визиты/оплаты кода не требуют.
...
- [ ] S2 (отмена): удалить → клик «Отменить» → строка вернулась → **ни одного удаляющего DELETE с телом** за окно (слушатель запросов: счётчик DELETE на `/records/` с непустым `postData()` = 0 — сам клик шлёт `?dry_run=true` DELETE без тела, панель wave 2: голый счётчик DELETE посчитал бы и dry-run) → reload → запись на месте; `queryDBRow` подтверждает.
...
- [ ] S6 (гонка): удалить чистую запись → в окне `page.request.post('/api/v1/payments', {data: {...}})` (тело — по образцу create из `test_api_payments.py`; ожидаемо 201) → `page.waitForResponse` на commit-DELETE → статус 409 → строка вернулась + error-тост с action-кнопкой «Обновить» (`[role="status"]`, текст «Не удалось удалить: данные изменились»; ассерт на кнопку — rev8; тост живёт 4500мс — кликать сразу, панель wave 2) → reload → запись и новый платёж живы (`queryDBRow` на обе таблицы).
...
### Steps
- [ ] `docs/domain-rules/records.md`: `:125` — §Delete **переписать** (лид-фраза «bare DELETE is a dry-run — 204 when no deps, 409…» после rev7 ложна): флаг `?dry_run=true` (pure preview: 204-без-удаления / 409 / 404 / 422-комбо), тело `{resolutions?, expected}` (**subset-сверка по id** — 409, когда на сервере есть зависимости, отсутствующие в expected; auto-зависимости из сверки исключены; порядок «сверка → валидация resolutions»; 409 `stale_dependencies`) и упразднением no-body (rev7): без флага и тела → 422 `expected_state_required`, тело без `expected` → 422; **отклонение контракта записей от общего паттерна** (no-body DELETE остаётся только у 5 архивных сущностей) — зафиксировать явно; `:126` — «Delayed delete: REMOVED» ревизовать: отложенное удаление возвращено (#285, решение юзера 16.09) на общем PendingActions-пайплайне (окно 5с, undo, expected-сверка) — не старым bespoke-механизмом; `:135` — Delete flow: новая механика хука (dry-run → диалог/deferred; commit с expected; undo = снапшот-restore); `:146` — строка таблицы API: `DELETE /records/{id}?dry_run=true` — preview.
- [ ] `docs/domain-rules/_overview.md`: `:108` — «dry-run preview (409)» → «no-body preview (409)»; `:160` — «An empty body (or no body) is the dry-run per §5 — produces 204 or 409, never executes» → «An empty/no-body DELETE never carries resolutions: execute-if-clean (204) or preview (409)» (уточнение двусмысленности, поведение не меняется). **Единый контракт (rev8):** короткая пометка — commit отложенного удаления несёт `expected` (подтверждённое пользователем серверное дерево; subset-сверка; `stale_dependencies` → честная ошибка + «Обновить»); **критерий подключения — наличие серверных зависимостей (FK_MATRIX), а не undo-окно или «не-архивность»** (панель wave 2; факт-коррекция rev9 от спеки #286): записи #285; визиты #297, оплаты #298 — листовые, expected не несут (наследуют только D4-обработку); занятия #286 — зависимости ЕСТЬ (каскад на записи, `activity.py:231-274`), в матрице их пока нет (каскад рукописный в сервисе) — Activity проводит в матрицу и подключает expected спека #286: строку контракта для занятий в `_overview.md` дописывает #286 пост-мержем (здесь её НЕ писать); Tag/Photo/Visitor/UserSettings — мгновенные hard-delete; архивным (staff/location/service/material/client) deferred не светит (решение юзера 17.09).
