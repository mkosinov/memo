...
- Фронтенд и `packages/api-client` не затрагиваются (подтверждено панелью: класс бэкенд-only).
...
## Task 3 — №4 фото: `list_photos_with_client`

classification: small
Required Docs: спека п.4; #263 (скоуп мастера, EXISTS-предикат).

- Свободная функция `list_photos_with_client(db_session, params, master_key)` в photo.py: скоуп-предикат EXISTS и условный outerjoin переезжают как есть; исполнение через `list_custom` (как сегодня).
...
## Task 5 — докстринги и полная верификация

classification: trivial
Required Docs: спека п.1, §«Тестирование»; ADR 007.

- Докстринг `list_clients_with_stats`: «future — #217» → «decision — ADR 007»; ссылка на #206 (обоснование дешёвого count) остаётся.
...
- Списочные композиты №2/№3/№4 исполняются через `list_custom`; №1 — задокументированное исключение; №5/№6/№7 — прямая сессия.
