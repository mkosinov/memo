...
- **Risk R-1: shard-start `rm -f` could delete the main dev `memo.db` if shard-start is misused.** Mitigation: the script already resolves `TEST_DB_PATH` to `test_memo_shard{id}.db` and the env-var is required. Add an explicit guard: `if [[ "$ABS_DB_PATH" == *memo.db && "$ABS_DB_PATH" != *test_memo* ]]; then echo "refusing to delete main dev DB"; exit 1; fi`.
...
- **Risk R-3: removing `_exists` could break developers who manually run `python -m seed.seed` on a populated DB.** Mitigation: document the "seed must run on empty DB" contract in `seed.py` module docstring. Add a tiny self-check at seed startup: if `Activity` table has any row → warn (don't throw, to allow legit manual re-runs). The warning is informational; globalSetup and shard-start are the real defenders of the invariant.
...
- **Risk R-5: dev DB (`memo.db`) gets touched by accident via shard-start.** Mitigation: shard-start requires `TEST_DB_PATH` env var. Add the same guard as R-1.