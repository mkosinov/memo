...
- **Visitor → Visit/Photo, Activity → Photo:** hard-deleting a Visitor or Activity **hard-deletes dependent Photos** (visitor_id/activity_id match) and a Visitor's dependent **Visits** — children of a hard-deleted parent must not be orphaned (photos/visits without their visitor/activity are meaningless in this domain). Implementation: explicit deletion in `VisitorService.delete` / `ActivityService.delete`; no DB-level FK enforcement is added (prod SQLite runs without `PRAGMA foreign_keys`). This policy decision is surfaced for G1b confirmation.
...
- **ActivityService.delete:** cascade per §1 — explicit deletion of the activity's Records (each record delete cascades to visits/payments) **and** dependent Photos (`photos.activity_id`), inside one transaction; ORM-level `cascade="all, delete-orphan"` / FK `ondelete="CASCADE"` added on the model as the integrity backstop. **Atomicity requirement:** the entire cascade must be atomic — do NOT call per-record `@transactional` methods in a loop (each commits mid-loop, `services/decorators.py:85`); perform deletions within the outer service transaction.
- **VisitorService.delete:** cascade — physically delete dependent Visits (`visits.visitor_id`) and Photos (`photos.visitor_id`) before deleting the visitor (per §1 policy; prevents orphans under prod SQLite where FK enforcement is off, and IntegrityError in tests where it is on).
...
- New tests: Activity delete cascades to Records (and transitively visits/payments) and Photos; Visitor delete cascades to Visits and Photos; atomicity (failure mid-cascade rolls back).
...
- [ ] Alembic migration drops `is_active` from the 6 tables; `alembic upgrade head` + `downgrade` both work (SQLite batch mode).
...
- [ ] DELETE Activity removes its Records (and transitively their visits/payments) and Photos — atomically; DELETE Visitor removes its Visits and Photos.
...
- **Data:** migration drops columns — existing archived (`is_active=false`) rows of the 6 entities will be kept as live rows after the column drop. Decision: keep them (they become ordinary rows); flag to user at gate if undesired. **Semantic ripple:** archived records with status in ACTIVE_RECORD_STATUSES become live and occupy seats in capacity checks — possible unexpected 409s on booking right after migration. Dev-instance impact likely minimal; noted for the gate.
- **FK integrity:** prod SQLite runs without `PRAGMA foreign_keys` while tests enable it — the service-level cascades (§1, §2.2) are the primary mechanism; ORM/FK cascade is the backstop. Never rely on DB-level FK alone.
...
- Downgrade re-adds `is_active` with `server_default=1` — previously archived rows come back active; unrecoverable by nature of a column drop (accepted, matches precedent).
