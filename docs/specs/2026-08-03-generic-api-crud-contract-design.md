...
- A wiring break (router not mounted in `main.py`, wrong `response_model`, POST returning 200 instead of 201, DELETE returning 200 instead of 204, wrong `<ENTITY>_NOT_FOUND` code) is caught only per-entity, by accident of duplication — and a **new** entity gets no wiring coverage until someone copies the tests.
...
- Extract to a shared module `backend/tests/generic_contract.py` (exact name/path finalized in plan, matching suite import conventions): `EntityConfig`, `CONTRACT_CONFIG`, `KNOWN_MISMATCHES`, `GENERIC_CONTRACT_EXCEPTIONS`, `_all_subclasses()`, `_contract_params()`, `_soft_params()`.
...
- **EntityConfig gains 3 fields** (HTTP-only; service contract ignores them):

| Field | Type | Semantics |
|---|---|---|
| `router_prefix` | `str` | e.g. `"/api/v1/masters"` |
| `not_found_code` | `str` | e.g. `"MASTER_NOT_FOUND"` |
| `response_schema` | `type` | `<Entity>Response` class — `model_validate()` on POST/GET/PUT bodies pins `response_model` wiring (Pydantic v2 ignores extra keys, so `ClientWithStats` extras in list items are tolerated) |
...
- **No multi-row seeding** (D4): list tests create exactly one row; deep pagination slicing is a service-level concern already pinned by #184. `unique_row_field` unused here.
- Update payload construction reuses the #184 rule: `{**create_data, **resolved_fks, **update_data}` filtered to `update_schema.model_fields`.

| Class | Tests (each parametrized ×8 unless noted) | Contract asserted |
|---|---|---|
| `TestGenericApiCreateContract` | `test_create_returns_201_with_valid_response_schema`; `test_create_response_fields_match_payload` | 201; `response_schema.model_validate(body)`; `id` present; body values for `create_data` keys equal sent values |
| `TestGenericApiGetContract` | `test_get_returns_created_entity`; `test_get_nonexistent_returns_404_with_entity_code` | create → GET → 200 schema-valid, fields match; GET `/nonexistent` → 404 + `detail.code == not_found_code` |
| `TestGenericApiListContract` | `test_list_envelope_exact_keys`; `test_list_contains_created_and_counts_total`; `test_list_pagination_params_echoed`; `test_list_invalid_params_returns_422` | 200; **exact** key set `{items,total,page,per_page}`; created id in `items`, `total ≥ 1`; `?page=1&per_page=5` → `page==1`, `per_page==5` (Query binding); `?page=0` → 422 |
| `TestGenericApiUpdateContract` | `test_update_returns_200_and_changed_fields`; `test_update_nonexistent_returns_404_with_entity_code` | create → PUT → 200 schema-valid; `update_data` fields changed; nonexistent → 404 + code |
...
**Estimated counts:** 116 new parametrized cases (create 16 + get 16 + list 32 + update 16 + delete 28 + patch-wiring 8) replacing ≈ 100 hand-written per-entity tests → net collected ≈ **+15**; backend suite ≈ 990p/5s → ≈ **1005p/5s** (TestClient + SQLite truncation; ADR 006 records the cost decision).

**Wiring self-guard (no separate guard test needed):** an unmounted prefix yields FastAPI's default 404 `{"detail": "Not Found"}` — the `detail.code` assertion fails. A dropped `response_model` fails `model_validate`. A status-code regression (200-for-201, 200-for-204) fails the status assertions. IMPL verifies with temporary mutations, reverted (§6).
...
| `test_api_tags.py` (120) | **all 120** | none — **file deleted** |
| `test_api_visitors.py` (257) | ≈165 | create with null age; PATCH `client_id` immutability; scoped-route regression (`/clients/{id}/visitors` stays a bare array) — note: the `GET /visitors` envelope tests (181–257) are now generic and are removed except the scoped-route assertion |
| `test_api_pagination_params.py` (39) | ≈25 (8 generic entities drop out) | visits-only 422/envelope params (visits is standalone, non-generic) |

**Sizing validation vs concept:** concept estimated "per-entity files slim by ~500 lines (create/get/list ~300 + update/delete ~200)". Measured: gross removal ≈ **930** lines (generic CRUD ≈ 1,010 across the 8 files was measured; some local fixtures/imports also die), new contract file ≈ 550 + shared-config delta ≈ 30 → **net ≈ −350…−450 lines**, consistent with the concept's ~500 order of magnitude. The concept undercounted gross duplication (it is ~2× the estimate) but the net lands in the promised range.
...
**ADR 006 content (deliverable, drafted in full so G1b approves the text):**

> **Status:** Accepted — **Date:** 2026-08-03
>
> **Context:** The HTTP layer of the 8 generic CRUD entities needed contract coverage (#185). Two candidate architectures: (1) endpoint-boundary tests with a mocked `GenericService` — fast, no DB, asserts the router calls the service correctly; (2) end-to-end tests through `TestClient` against the real test SQLite — the full request path: routing, DI binding, schema validation, `response_model` serialization, service, repository, DB.
>
> **Decision:** Adopt (2) end-to-end TestClient + test SQLite. Rationale: the dominant HTTP-layer risk in this codebase is **wiring across 8 entities** — per-entity router prefix mounting in `main.py`, per-router DI bindings, `response_model` on each route, status codes (201/200/204), per-entity `<ENTITY>_NOT_FOUND` error codes (ADR-005 body shape). A mocked service verifies none of that. The marginal cost of SQLite is low (~1–2 s on the whole set; the suite already standardizes on `api_client` + per-test truncation). Semantic depth (soft-delete rules, is_active stickiness, pagination slicing) is deliberately **not** re-asserted here — the service-level contract (#184) owns semantics; this contract owns transport only.
>
> **Consequences — Positive:** router miswiring fails loudly per entity; a new generic entity gets full HTTP+service coverage from one config entry; ~1,010 lines of duplicated HTTP tests collapse into one parametrized file; the two contract levels are complementary, not overlapping. **Negative:** contract cases run through the DB (suite +~15 net cases, still ≈5 s); a transport failure can theoretically mask as a service failure — triage rule: if the HTTP contract fails and the service contract is green, the bug is in routing/schemas. **Risks:** future non-uniform endpoint (a third response shape) breaks the exact-envelope assertion — that is the contract working as intended; add the entity to exceptions only with a documented reason.
>
> **References:** Issue #185; spec `docs/specs/2026-08-03-generic-api-crud-contract-design.md`; #184 service contract spec; ADR-005 (error contract codes); #175 patch contract.
...
3. Developer drops/alters `response_model` on a route → `response_schema.model_validate` fails.
...
2. Shared config extracted to `backend/tests/generic_contract.py`; `EntityConfig` gains `router_prefix`, `not_found_code`, `response_schema`; all 8 entries extended; `test_generic_service_contract.py` imports from it and its suite stays green (pure move).
3. Per-entity disposition per §3.4 executed; `test_api_tags.py` deleted; `test_api_pagination_params.py` slimmed to visits; 8 PATCH-wiring remnants absorbed (unless G1b vetoes D6).
4. Net line delta ≈ −350…−450 (`git diff --stat` on `backend/tests/`).
5. IMPL-verified mutations (all reverted): (a) unmount one router prefix → 404-code test fails; (b) drop `response_model` from one route → schema validation fails; (c) flip one status code → status assertion fails.
6. Full backend suite green (≈ 1005p/5s); no production files changed (`git diff --name-only backend/src/` empty).
7. `docs/decisions/006-http-contract-tests-end-to-end.md` committed per TEMPLATE.md (content ≈ §3.6 draft); `docs/decisions/README.md` index gains the missing 005 row + 006 row.
