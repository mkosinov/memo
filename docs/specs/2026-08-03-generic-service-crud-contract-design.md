...
- **Date:** 2026-08-03 (rev 4 — spec-panel round 2 fixes: concrete strip mechanism D13, omission-test pop, arithmetic, doc addenda)
...
| `TestGenericServiceUpdateContract` | `test_update_applies_update_data`; `test_update_nonexistent_returns_none`; `test_update_omitted_optional_field_reverts_to_default` | create → update → get: `update_data` fields changed **and** non-`update_data` create fields unchanged; nonexistent → `None`; **PUT discrimination:** set `nullable_field` to non-default, build the standard payload and **explicitly pop `nullable_field`** (it otherwise rides along via `create_data` for Client/Activity), update → field reverts to `update_schema` default (Tag, Material skip with reason — no `nullable_field`) |
| **`TestGenericServiceIsActiveContract`** (NEW — soft entities only, `delete_semantics == "soft"` → Master, Location, Material, Client) | `test_get_archived_returns_row_with_is_active_false`; `test_update_preserves_is_active_when_omitted`; `test_update_explicit_is_active_applies`; `test_patch_preserves_is_active_when_omitted_or_none`; `test_patch_explicit_is_active_applies` | **User decisions 1-3** (multi-phase tests with labeled assertions for failure localization): get(archived) → row (not None) with `is_active is False`; update archived without `is_active` → still False, update active without `is_active` → still True; update active + explicit `is_active=False` → archived, update archived + explicit `is_active=True` → **reactivated and visible in `list()` again**; patch without `is_active` **or with `is_active=None`** → preserved (both variants); patch active + explicit `False` → archived, patch archived + explicit `True` → **reactivated and visible in `list()` again** (both directions pinned — only Master had an API-level True-restore test before) |
...
**Service-layer changes (`backend/src/services/`):** the hook lives in the **service layer**, honoring the #195 design rule "base `GenericService` has NO `is_active` knowledge" (`generic.py:51-52`). **D13 — concrete mechanism** (panel-verified: plain "strip + delegate" is NOT implementable for update, because base `GenericService.update` passes the schema object to `repo.update`, which re-dumps it internally (`repositories/generic.py:68`) and would re-inject `is_active=None`):

- **`SoftDeleteService.update`** (`generic.py:155-179`, new override): if the update schema has an `is_active` field and its value is `None` → fetch the current row, then `data.model_copy(update={"is_active": current.is_active})` → `super().update(...)`. Stored-value **injection** (not stripping): full-replace semantics stay literally intact — the base re-dump now carries the stored value instead of the old `True` default. Double fetch (service get + repo fetch) is acceptable for admin CRUD within one transaction.
- **`GenericService.patch` hook extraction** (`generic.py:115-136`): the existing `model_dump(exclude_unset=True)` + NOT_NULL-strip logic moves into a `_patch_payload(data) -> dict` method on the base (still zero `is_active` knowledge — mechanical refactor, existing patch contract tests guard it). **`SoftDeleteService._patch_payload`** overrides it to additionally pop `is_active` when `None`.
- **Shared helper** `_strip_is_active_none(d: dict) -> dict` in `services/generic.py` — single implementation used by `SoftDeleteService._patch_payload` **and** `ServiceService.update`/`patch` (`service.py:110-206`, which build apply-dicts via `model_dump(exclude={...})` + `setattr` without calling `super()`). Prevents the drift that let the hazard live in ServiceService in the first place (panel finding). `ServiceService.update`'s dict path pops `is_active` when `None` via the helper (its full-dump already carries every other field, so full-replace semantics are preserved there too).
- Docstrings on all three touchpoints document `is_active` as a **sticky-field exception to PUT full-replace** (like `id`/`created_at`).
...
- **`docs/domain-rules/_overview.md`:**
  1. New section **"is_active semantics on get/update/patch"** under the Deletion Policy: (1) `get` returns archived rows, `is_active` exposed in Response schemas (*list hides, get returns*); (2) update/patch preserve `is_active` when absent/None; explicit bool applies; `True` on archived = legal reactivation; (3) `is_active` is a sticky-field exception to PUT full-replace.
  2. PATCH Contract None-handling table (:89-95) gets an explicit **third row**: "`is_active: None` (soft-delete entities) → stripped, stored value preserved (sticky field — see is_active semantics section)" — it fits neither existing row (NOT NULL strip / nullable null-write).
  3. One line: **create always yields `is_active=True`** — Create schemas do not expose the field and the strip does not apply on the create path (a future Create-schema `is_active` field would bypass sticky semantics — flagged as a trap).
  4. One line: **reorder silently skips archived rows** (`repositories/generic.py:169`) — interplay with reactivation documented.
  5. Update the stale file reference at `_overview.md:97` (rename).
- **Per-entity files** (`masters.md`, `locations.md`, `materials.md`, `clients.md`, `services.md`): short "Archive semantics on write" section each, referencing the general rule (+ entity notes: Client gains `is_active` in Update/Patch; Service implements the strip in its own overrides). **`clients.md:30-32` is explicitly updated**: the current framing implies the `ClientPatch` schema omission blocks restore — after this change the schema gap is closed and only the frontend restore-buttons UI remains a follow-up.
- **Addendum note** in `docs/specs/2026-08-02-is-active-list-filters-design.md` §3 (one line): the "Backend Update-schema `is_active: bool = True` default — out of scope" item is now addressed by #184 §3.5.
...
5. **`TestGenericServiceIsActiveContract` green (5 tests × 4 soft entities); update-preservation test fails if the stored-value injection is removed from `SoftDeleteService.update`, patch-preservation test fails if the `_patch_payload` pop is removed (IMPL-verified mutations, reverted). Same 5 semantics green for Service via `test_service_service.py`.**
...
- **D13 (panel blocker fix):** concrete strip mechanism — `SoftDeleteService.update` uses **stored-value injection** (`model_copy(update={"is_active": current})` when payload `is_active is None`) and delegates to base (base re-dump then carries the stored value); plain "strip + delegate" is impossible because base `update` re-dumps the schema internally. `patch` uses a `_patch_payload` hook extracted from base (stays is_active-agnostic); `SoftDeleteService._patch_payload` and `ServiceService.update/patch` share the `_strip_is_active_none` helper (single implementation, no drift).
- **Panel conflict resolution (round 2):** simplicity suggested merging the is_active preserve/explicit test pairs; best-practices flagged the same two-phase tests as Assertion Roulette. Resolution: **keep 5 tests** with multi-phase **labeled assertions** (assert messages per direction) — failure localization without test-count explosion. Simplicity's "fold get-archived into DeleteSemantics" dismissed: the get-archived test pins user decision 1 and belongs with the directive-semantics class (discoverability, scenario 5).
- **Panel suggestions dismissed (round 1):** single-sentinel update instead of `update_data` (concept-mandated, weaker); 2-representative pagination (defeats the pattern; real `list` overrides exist); merging create-persists into get test (resolved via ORM-level path instead).
