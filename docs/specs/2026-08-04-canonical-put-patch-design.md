...
| same file, `TestGenericServiceUpdateContract` (~:738-790) | update payloads gain explicit `is_active` for **all soft entities including Client** (uniform — Client needs it now: omitting → 500). Injection is **entity-conditional** (guarded by `update_schema.model_fields`) in the shared `_update_kwargs` payload helper (:72) — hard entities unaffected; the flipped tests construct the is_active-less variant explicitly (pop) to assert `ValidationError` |
