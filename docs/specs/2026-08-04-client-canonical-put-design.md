...
| Frontend admin PUT payload (`ClientInfoTab.tsx:58-61`) | `{name, phone, email, channel}` untyped (`Partial<ClientWithStats>`), channel `''` for «Не указан», no `is_active` | Payload explicitly typed **`ClientUpdate`**: all 4 fields + `is_active: client?.is_active ?? true` (create mode routes the same `save()` with `client={null}`; create path ignores it — panel B2); empty strings convert `'' → null` for name/phone/email; channel passes a **save-time allowed-set conversion** (unknown/legacy → null, §3.3) |
...
**Uniformity note:** with this change, **all 5 soft-delete entities require `is_active` on PUT** (omission → 422) — the `_overview.md` Client exception is removed. Client additionally requires the 4 nullable personal keys on PUT; the 4 #178 entities keep defaulted optional fields (omission → default reversion, not 422) (panel m1 wording).
...
1. `ClientInfoTab.tsx` (`app/(main)/clients/components/`):
   - Channel state init (:28): `client?.channel || ''` → display-normalize values outside `{telegram, whatsapp, max}` to `''` (select honestly shows «Не указан» for legacy rows instead of a blank uncontrolled-looking select). The `useEffect` re-sync (:42) and `handleCancel` (:67) re-set raw `client?.channel` and stay raw — correctness is enforced at save time (next bullet), not at every state-set site (panel M2).
   - `onSave` prop type (:16): `Partial<ClientWithStats>` → `ClientUpdate`.
   - `handleSave` (:58-61): build an explicitly typed `ClientUpdate` literal — `{ name: name || null, phone: phone || null, email: email || null, channel: ['telegram','whatsapp','max'].includes(channel) ? channel : null, is_active: client?.is_active ?? true }` (every field listed; compile-time completeness guarantee). The **save-time allowed-set conversion** covers every state-set path (:28 init, :42 effect, :67 cancel) at a single point — a legacy value reaching save becomes null instead of a 422 (panel M2; simpler than 3-site normalization). `is_active: client?.is_active ?? true` because create mode (`ClientCardModal` «Создать» → same `save()` ref, `client={null}`, `page.tsx:100-116`) shares this handler — without the fallback it is a TS error + runtime TypeError (panel B2); the create path ignores `is_active`. Channel state retyped/narrowed so the literal type-checks (`tsc` — `string | null` is not assignable to the enum union as-is).
...
3. `ClientCardModal.tsx:193`: `async (data: any)` → `async (data: ClientUpdate)`; drop the `as any` at :187 that currently hides the create-path payload (panel B2 follow-through).
...
**G1b panel extension of the inventory:** the grep above cannot catch fallout without `#201` markers — the spec-review panel (2026-08-05) added three such spots, marked "panel finding" below: shared contract payload helpers (B1), `test_schemas_client.py` (M1), `test_put_invalid_channel_returns_422` payload (m3). Plan step greps once more before implementation.

**Backend:**

| File:lines | Change |
|---|---|
| `tests/services/test_generic_service_contract.py` `TestGenericServiceIsActiveContract.test_update_without_is_active_raises_validation_error` (:862-889) | The Client skip is **data-driven and self-lifting** (`if not field.is_required(): pytest.skip("GH #201…")`) — it lifts automatically once `ClientUpdate.is_active` is required. Remove the now-dead executable guard (:880-881 `if not field.is_required():`) + stale `#201` comments (:826-829, :871-873, :882); **verify the Client param runs green** (skip count drops 6 → 5) |
...
| `tests/test_api_clients.py` `test_put_sets_all_nullable_to_null` (:190-206) | **Stays 200, promoted from edge case to canonical pin** (scenario 3): full body with explicit `null` in all 4 fields + `is_active` → fields become NULL, stats/relations intact. **Panel finding (m4):** today the client has no records/payments so "stats intact" is vacuous — **seed a record + payment** before the wipe so `records_count`/`total_paid` continuity is asserted, not just documented. Docstring updated accordingly |
| `tests/test_api_clients.py` `test_put_invalid_channel_returns_422` (:209, payload :214-217) | **Pin fix (panel m3):** add `"is_active": True` to the body — otherwise post-change it 422s for *missing `is_active`* instead of the invalid channel and silently stops testing what it claims. Continues to assert invalid channel → 422 |
| `tests/test_api_clients.py` `TestClientChannelTolerance` (:359-390), PATCH cases (:173-181) | **unchanged, stay green** — GET read tolerance for legacy values preserved; PATCH contract untouched |
| `tests/generic_contract.py` Client `CONTRACT_CONFIG` (:185-198) + shared PUT helpers `_update_payload` (`test_generic_api_contract.py:50-58`) / `_update_kwargs` (`test_generic_service_contract.py:73-94`) | **Panel finding (B1) — spec's earlier "keeps working" claim was wrong:** helpers build Client PUT bodies from `create_data`+`update_data` = `{name, phone}` + injected `is_active` — missing `email`/`channel`, which become required keys → ≥5 Client-param tests go red (HTTP `test_update_returns_200_and_changed_fields`, `test_update_nonexistent_returns_404_with_entity_code` → 422 instead of 200/404; service `test_update_applies_update_data`, `test_update_nonexistent_returns_none`, `test_update_explicit_is_active_applies` → `ValidationError`). **Fix:** enrich Client `create_data`/`update_data` with `email`+`channel` (preferred — keeps helpers generic) so every required `ClientUpdate` key is present; verify the 5 params green **and** that `test_update_without_is_active_raises_validation_error` still fails for the right reason (missing `is_active`, not personal fields). Plus comment cleanup of the stale window note (`test_generic_api_contract.py:55`) |
| `tests/test_schemas_client.py` `TestClientUpdateNullableFields` (:76-88) | **Panel finding (M1 — missed by grep inventory, no `#201` marker):** `test_update_with_all_fields` (`ClientUpdate(name, phone, channel)` — no `email`/`is_active`) and `test_update_with_no_fields` (`ClientUpdate()`) raise `ValidationError` post-change. **Rewrite both** to pin the required-5-key contract: omitted keys (any of the 5) → `ValidationError`; full 5-key payload with explicit nulls → valid |
...
| File | Change |
|---|---|
| `schemas.test.ts` | **ADD `ClientUpdateSchema` group**: full-payload accept (values + nulls); reject missing `is_active`; reject one representative missing personal field (zod required keys are declarative — one case pins the mechanism; panel m6 trim); accept all-null clear + `is_active`; reject invalid channel string |
...
| `__tests__/ClientsIntegration.test.tsx` :796-836 ("handles duplicate phone on create") | **Verify green, no edit expected** (panel B2 guard): exercises `save()` with `client={null}` (create mode) — pins the `client?.is_active ?? true` fallback; must be in the run after the `handleSave` change |
...
**Estimated delta:** backend ≈ +2-3 cases (new 422s; B1 helper data enrichment, M1 schema-test rewrite, m3/m4 payload/seed edits are in-place, ±0 count), −1 skip (self-lifted) → ~1002p/5s; api-client ≈ +6-7 cases (m6 trim applied); admin vitest ±0 (in-place assertion updates); e2e −1 skip.

### 3.5 Domain rules (lands in the feature branch)

- `docs/domain-rules/_overview.md` :79 — PUT bullet: Client joins the canonical list — all 5 soft entities require `is_active` on PUT (omission → 422); Client additionally requires the 4 nullable personal keys (the 4 #178 entities keep defaulted optional fields — panel m1 wording). **Delete the Client exception / #178→#201 window note.**
- `docs/domain-rules/clients.md`:
  - :78 "Archive semantics on write" — rewrite: window note replaced by the new canon — `ClientUpdate` = 4 required-nullable fields + required `is_active`; explicit `null` = deliberate clear (data-wipe semantics: personal fields erased, payments/stats by `client_id` joins intact); PATCH sticky unchanged. **Panel M3 additions:** explicitly document that the PATCH media type is plain `application/json` (**not** `application/merge-patch+json` — sticky `null` → preserve deviates from RFC 7396, avoiding misleading OpenAPI/codegen consumers), and that the same JSON `null` carries divergent semantics — **clear on PUT, preserve on PATCH** (footgun note for API consumers).
...
- **`handleSave` `is_active` fallback** (G1b panel B2): `client?.is_active ?? true` — create mode routes the same `save()` ref with `client={null}`; the create path ignores `is_active`.
- **Channel normalization at save time** (G1b panel M2): single allowed-set conversion (`['telegram','whatsapp','max'].includes(channel) ? channel : null`) covers all state-set paths (:28/:42/:67); display-only normalization stays at init :28. Alternative rejected: normalize at all 3 sites (more code, same guarantee).
...
- [ ] AC3: `ClientInfoTab` handleSave builds an explicitly typed `ClientUpdate` (all 5 keys; `'' → null`; save-time allowed-set channel conversion; `is_active: client?.is_active ?? true` — create-mode-safe); `ClientsContext` update path + `ClientCardModal` onSave typed `ClientUpdate`; `tsc` clean.
...
- [ ] AC5: Backend suite green incl.: `PUT {} → 422`, `PUT` minus `is_active` → 422, `PUT` minus one personal field → 422 (HTTP + service-level pins), explicit-null wipe → 200 with NULLed fields **and seeded record+payment stats asserted intact** (m4), invalid channel → 422 **with `is_active` present in the pin payload** (m3), legacy-channel GET tolerance, PATCH sticky cases, generic contract suites green with enriched Client payload data (B1), `test_schemas_client.py` required-5-key pins (M1).
...
- **G1b spec-review panel applied (2026-08-05):** 5/5 perspectives; 2 BLOCKER (B1 contract-helper payload gap, B2 create-mode `handleSave` crash) + 3 MAJOR (M1 missed schema tests, M2 channel normalization hole, M3 null-semantics doc) + minors — all applied in rev 2, marked "panel …" in §2/§3.3/§3.4/§3.5/§5/AC. Core design validated by all perspectives (best-practices verified Pydantic v2 required-nullable + zod idioms against current sources).
