...
- `withStatus` / `withSearch` are explicit parent-controlled props; DataTable does not infer them from `tableState` (both default false; matrix below).
- **`withSearch` / `withStatus` per-entity matrix (verified, corrected at plan review iter 2):** standalone in-table search box exists today ONLY in Tags ("Поиск тегов...") and Photos ("Поиск фото...") → `withSearch: true` for these two. Locations/Masters/Materials/Services search lives in their `*Filters` bars (controlled components; state moves to the factory, §6.7 — UI pixel-unchanged) → `withSearch: false`. Clients search lives in the page-level `ClientsFilters` bar (`🔍 Поиск по имени или телефону`, debounced, wired to `setFilters`) → `withSearch: false`. Records: no search → false. **`withStatus: false` for ALL 8 tables:** the 4 dict bars already own their Статус select wired to context `status`/`setStatus` (verified LocationsTable.tsx:51-57,:254 — no rewire needed); Clients' page bar owns its status select; Tags/Photos/Records have none. The `withStatus` prop stays in the contract per locked DoD #2 (its false-default IS the bug-proofing) but is initially unused — DataTable implements the select for contract completeness.
...
| `status` / `setStatus` | **no exposure needed** — DataTable `withStatus: false` everywhere (§6.1 matrix); the page-level ClientsFilters bar keeps using `filters`/`setFilters` | `filters.status` is a **booking-status string**, NOT ArchiveFilter — context OMITS the optional fields |
...
- **Search mechanics (D3, locked):** debounce 300ms + Enter-to-submit-immediately (Enter submits the current value and cancels the pending debounce). DataTable owns the input, rendered only when `withSearch` (§6.1 matrix: Tags/Photos only). `PagedListState` gains **optional** `search`/`setSearch`.
...
| Empty | `items.length === 0 && !isPending && !error` | Empty row: "Нет записей" for ALL 8 tables — **Policy (USER RULING 2026-08-20, mid-IMPL; overrides the earlier G1a/G2 per-entity-copy pin for empty states ONLY):** the DataTable default covers every table; wrappers do NOT pass `emptyLabel`. Clients' dual empty variants (reset button «Сбросить фильтры» + SVG icon) are DROPPED — the reset button lives in the filters bar, no functional loss. The `emptyLabel` prop stays in the contract (§6.1) as an optional override, unused by the 8 migrations. Previously-pinned per-entity copy ("Теги не найдены", "Локации не найдены", "Мастера не найдены", "Материалы не найдены", "Услуги не найдены", "Ничего не найдено", "Фото не найдены", "Записи не найдены") is thereby retired |
...
- **B2 audit (grep before each migration):** categories of intentional per-suite edits:
  ...
  14. **Empty-label copy — EDITS EXPECTED (USER RULING 2026-08-20, §6.8):** all 8 tables unify on "Нет записей" (DataTable default; wrappers do not pass `emptyLabel`). Unit/e2e assertions on per-entity empty copy ("Теги не найдены", "Ничего не найдено", …) and Clients' empty-state reset-button/SVG are updated to the unified copy — these ARE sanctioned edits. Applied retroactively to T1–T5 in one standalone alignment commit before T6.
...
- Capture baselines for **all 8 tables BEFORE T1**. Baseline states per table (7): skeleton / filled / empty / error / dropdown-open / picker-open / sort-active. Toolbar layout differs per entity (standalone search box on Tags/Photos; Filters bars elsewhere) — capture each table in its own variant (no separate per-variant doubling; 7 states × 8 tables ≈ 56 baselines).
...
13. Plan-review grounding corrections (2026-08-18): (a) the earlier "4 dicts have NO search UI" matrix claim was wrong — Locations/Masters/Materials/Services search lives in their `*Filters` bars (§6.1 matrix corrected; resolution keeps Filters UI pixel-identical and moves only the state/predicate into the factory); (b) T7 Photos dependency resolved at plan time: #206 did NOT deliver a Photos context and #211 (photos server pagination) is still open — plan Task 7 uses a factory-based **client-adapter** context (full array → sort/filter/slice locally) so T7 stays unblocked; #211 later swaps fetcher internals only. This is subject to G2 confirmation (alternatives: gate T7 on #211, or defer T7 to a follow-up PR). (c) Plan-review iter 2 — status/search ownership: dict `*Filters` bars already wire their Статус select to context `status`/`setStatus` (verified LocationsTable.tsx:51-57,:254), and Clients' page-level `ClientsFilters` owns search+status → `withStatus: false` for ALL 8 and `withSearch: true` only for Tags/Photos, eliminating duplicated controls (§6.1 matrix final).
