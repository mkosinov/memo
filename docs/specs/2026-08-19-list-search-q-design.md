...
| id semantics | when `q` parses as a **full UUID** (canonical 36-char form), an exact-equality `id == q` clause is OR'd in. Partial id strings never match id (they only hit the substring fields, which normally won't match hex). This is the #216 deep-link prerequisite (a UUID is 36 chars — `min_length=2` is no obstacle). |
...
1. **One pure helper** — `backend/src/repositories/search.py` (NEW):
   ...
   - `search_predicate(q, fields) -> ColumnElement[bool]`: builds the OR of ilike clauses (with `%`/`_` escaping + `ESCAPE` clause) and, when q is a full UUID, the `id == q` equality. Pure function, unit-testable without a DB.
2. **Generic path:** `BaseRepository.list` / `ArchiveRepository.list` gain optional `q: str | None` + `search_fields: Sequence[SearchField] | None` params; predicate applied to `stmt` **before** COUNT (count consistency free). Services pass `self.search_fields` through.
   - Service-level rule: `GenericService.list`/`ArchiveService.list` accept `q` and forward `q` + `self.search_fields` to the repo. A service without declared `search_fields` never receives `q` from its router (the param simply isn't declared there) — nothing else needed; `search_fields` defaults to `None` and is then required iff the router declares `q`.
3. **Custom paths:**
...
7. **Activities list response:** `ActivityResponse` gains `service_title: str` (via the Service join / relationship — implementation detail for the plan; must be present for PhotoModal's display switching). Bounded-query-count guard: `test_list_activities_query_count.py` must stay green (eager/column strategy chosen accordingly).
...
- **Step 1 — probe test (written FIRST, TDD):** insert `Иван`, search `иван` via a list endpoint → establish truth. Expected to fail pre-fix.
- **Step 2 — fix (~10 lines):** inside the existing connect listener (`db/database.py:16-44`, after the sqlite guard), `dbapi_connection.create_function("lower", 1, lambda s: s.lower() if s is not None else None)` — registers Python's full-Unicode `str.lower` as SQLite `lower()` per connection. Covers all engines (app async, alembic sync, sqladmin sync) since the listener is shared.
...
4. **Clients:** `ClientsFilters` bar unchanged visually; context filter key renamed `search` → wire param `q` (frontend state naming decided at plan level — minimal churn). Placeholder "🔍 Поиск по имени или телефону" stays (email searchability is a bonus, copy unchanged).
...
6. **PhotoModal typeaheads** (4) migrate to list `?q=` with `per_page=10`:
   ...
   |---|---|---|---|
   | services | title ilike **+ is_active filter**, limit 10 | `getServices({q, per_page:10})` — default `status=active` | ✅ preserved (default = old filter) |
   ...
   | activities | service.title ilike, optional service_id, limit 10, **start pre-formatted "%H:%M %d.%m.%Y"**, service_title included | `getActivities({q, service_id?, per_page:10})` — response ISO `start` + guaranteed `service_title` | ⚠️ accepted delta: frontend closure formats `start` for display (formatting util chosen at plan level); service_title/service_id contract preserved |

   - Mapping to `SearchItem` (`{id, …displayFields}`) happens in the PhotoModal `onSearch` closures (`r.items.map(...)`); SearchableSelect itself is untouched except the clamp.
...
4. **1-char query never reaches the server as a valid request.** UI: SearchableSelect does not fire on 1 char; table search boxes fire (they search ≥2… — note: table inputs are free text; a 1-char table query **will** fire and receive 422 → the factory must treat 422-on-q as a displayed error state, same as any query failure — OR clamp table inputs too. **Decision: table search boxes also wait for ≥2 chars before firing** (uniform UX, no 422 noise; enforced in the factory/DataTable search wiring). API-level 422 below 2 chars is still contract-tested. → unit tests (factory clamp) + backend 422 contract tests.
...
| 12 | activities: q + service_id combo; `service_title` present in every item | PhotoModal contract |
| 13 | tags | the full matrix (gap closure — zero tests today) |

Plus endpoint-level: `GET /clients/get?phone=` — exact match 200; partial → 404; archived → 404; `min_length=3` → 422; route shadowing check (`/clients/get` resolves to the phone route, not `/{client_id}`).
...
| Atomic rename search→q breaks a missed consumer silently | Grep-driven call-site audit in the plan (api-client passthrough means TS won't catch a stale `search` key); contract tests assert `q` honored and `search` ignored. |
| Activities `service_title` addition breaks query-count guard | Choose column/eager strategy to keep `test_list_activities_query_count.py` green; verified in review. |
