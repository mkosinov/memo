...
- **Future cleanups (explicitly out of scope, panel B2/F3):** (a) derive the deep-link search from the URL during initial render (URL-as-source-of-truth) to kill the wasted default-key round trip on mount — filed as #231; (b) if a future surface needs deep-linking at scale, revisit a dedicated `?id=` + page-resolution endpoint instead of the UUID-in-search-box compromise — filed as #232.
