...
**Scale (panel-verified): 24 test files reference `useSchedule`/`ScheduleContext`/`ScheduleProvider`** — all migrate. Groups: `__tests__/ScheduleContext.test.tsx` (717 lines — splits per destination context); component tests mocking `useSchedule` (`Topbar`, `CellHeight.Topbar`, `Toolbar`, `Menubar`, `StampPanel`, `ActivityCard`, `DayView`, `WeekView`, `DayColumn`, `OverlapPopover`, `ActivityDetailsModal`, `optimisticUpdate`, `scheduleIntegration`, `page`, `ClientsPage`, `ClientsIntegration`, `ClientTab.integration`, `ClientRecordTab.{interactions,api,layout}`); shared helpers `__tests__/helpers/mockContexts.ts` and `__tests__/helpers/renderWithProviders.tsx` — the single `useSchedule` mock splits into three (`useScheduleData`/`useScheduleView`/`useGridSettings`). New suites: `usePersistedState.test.ts`, guard tests, zoom-isolation test. E2e suites referencing `__memo-*` events keep working (events unchanged); no new e2e required for С1/С2/С3 (unit-level).
