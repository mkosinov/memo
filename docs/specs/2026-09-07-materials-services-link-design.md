...
- **`material_hint` consumers (complete map, panel-verified):**
  ...
  - `frontend/web` (client app): `lib/mappers/buildSchedule.ts:81` (`materialHint`) AND `:90` — derived single string `material = material_hint.split(',')[0]` flowing into `page.tsx:57,69` (card label + details), `ui/ActivityDetail.tsx:127,149` (chip text + grouping key), and `ui/BookingPrivateOverlay.tsx:91,181-182` — an **editable «Материал» booking-form field prefilled from it**; plus `lib/model/dto/schedule.ts` tests;
