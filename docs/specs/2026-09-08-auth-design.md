...
- **Status:** DESIGN phase, G1a passed 2026-09-08; G1b revision 1 (5-reviewer panel + user decisions) same day; G2 correction — §3.9 aligned with the real sqladmin 0.20 API (`AuthenticationBackend(secret_key=...)`, `form_overrides` + `on_model_change` instead of `scaffold_form`)
...
9. **User provisioning:** a small CLI (`python -m src.cli create-user --phone ... --role admin|master`, password via double `getpass` prompt) creates the first admin — **no default passwords committed** (the repo is public). Flags kept minimal (`--phone`, `--role` only; G1b simplification — linking a user to a staff card — and the later master section — happens in sqladmin). Dev seed additionally creates two demo users (admin + master linked to the first seeded staff card) with fixed obviously-dev passwords, guarded by `ENV != production`.
...
11. **Brute-force throttle on login (G2 re-scoped by user decision): per-phone escalation ladder.** 3 failures → **15-minute lock**; after it expires, another 3 failures → **1-hour lock**; after that, another 3 failures → **hard lock until an administrator resets it** (sqladmin: clear the lock fields on the user). The ladder state lives on the `users` row — `failed_login_attempts int default 0`, `lock_level smallint default 0` (0 none / 1 timed-15m / 2 timed-1h / 3 hard), `locked_until datetime nullable` (NULL for the hard lock) — so it survives restarts and is admin-resettable. A locked account gets 429 `AUTH_LOCKED_OUT` **even with the correct password**; timed locks carry `Retry-After`, the hard lock carries none and its message says an administrator reset is required. A successful login resets `failed_login_attempts` and `lock_level` to 0 — the hard lock is the only state nothing but the admin clears. A **secondary per-IP counter** (≥20 failures within 15 min across any phones) stays in-memory (single-process runtime by design, #239-verified) and slows credential stuffing across many accounts.
12. **Frontend:** `/login` page (phone + password, controlled inputs — the repo has no form library), `AuthContext` in providers (`/me` on mount), `api()` gains `credentials: 'include'` plus a registrable 401 handler (admin registers a redirect to `/login?returnTo=…`; the shared client stays framework-neutral so `frontend/web` is unaffected). Client-side guard in the `(main)` layout. **The interface is identical for both roles in v1** (user decision, G2: per-role display tuning — hiding sections/buttons for masters — is a later task; the API 403 remains the enforcing layer, forbidden writes surface as the standard error toast). The existing bottom-left user block in the sidebar (`frontend/admin/app/components/layout/Menubar.tsx` «User Avatar», today hardcoded «А»/«Админ») is wired to the real logged-in user — staff card name or phone, role label, avatar initial — and hosts the logout control. `AuthContext` ships `can(permission)` for the future tuning; no element is filtered by it in v1. `UserSettingsContext` switches from `DEV_USER_ID` to the session user's id (closes the existing TODO). No Next.js `middleware.ts` — the session cookie is opaque (edge runtime cannot verify it against the DB), so the guard is client-side plus the 401 interceptor.
...
One alembic revision creates the `sessions` table **and adds the user-lock columns to `users`** (`failed_login_attempts int default 0`, `lock_level smallint default 0`, `locked_until datetime nullable` — the §2.11 ladder state; `backend/src/models/user.py` gains the three columns). Constants live in the auth package (not settings — G1b simplification): `IDLE_WINDOW = 7 days`, `ABSOLUTE_CAP = 30 days`, extension throttle 1 hour. Hard-deleted rows (logout/expiry); opportunistic cleanup deletes expired rows for the user on login. Multiple concurrent sessions per user allowed.
...
- `login(phone, password, client_ip) -> (User, token)`: ladder check first (`lock_level == 3` → hard-locked 429; `locked_until > now` → timed 429 with `Retry-After`), then per-IP check → normalize phone (trim) → fetch by phone **with `is_active == True`** (archived users cannot log in) → `verify`. **Timing parity:** when the user is not found, run `verify` against a module-level dummy Argon2 hash anyway, so both branches take similar time. Failure: `failed_login_attempts += 1`; on reaching 3 → `lock_level += 1` (capped at 3), set `locked_until` (now + 15 min / now + 1 h / NULL for hard), reset `failed_login_attempts` to 0, raise `AUTH_LOCKED_OUT`; below 3 → raise `AUTH_INVALID_CREDENTIALS` (identical for unknown phone and wrong password — no user enumeration). Success (never reached while locked): `failed_login_attempts = 0`, `lock_level = 0`, **deletes any `memo_session` row presented in the request** (session-id rotation on privilege change, OWASP), creates the session row, runs opportunistic cleanup.
...
Lockout state: the per-phone ladder lives on the `users` row (§2.11, DB-backed); the per-IP secondary is a module-level `dict["ip:<ip>", (count, window_start)]` (threshold 20, window 15 min).
...
`user` = `{id, phone, role, master_id, email}` (id is the users.id UUID string — feeds user-settings). The `master_id` key is kept **unchanged for frontend compatibility** (form response unchanged, #266 D10), but now carries the internal `users.staff_id`; the auth principal is `AuthedUser.staff_id` (renamed from `master_id`). `master` = display snapshot assembled from the linked staff card (`{first_name, last_name, avatar_url}` — `avatar_url` added by #262 amendment, 2026-09-10) plus master fields when the master row exists, else `null` — the sidebar user block (§4.5) shows «Аноним» when `master` is null (a user without a linked staff card is valid and must not break the UI). Registered in `main.py` (`prefix="/api/v1/auth"`); all three are in `PUBLIC_ROUTES` (`me` returns 401 rather than requiring the dependency — the frontend must distinguish "no session" from "wrong password"; `getMe()` therefore resolves 401 to `null`, not an exception — §4.1).
...
- `class SqlAdminAuth(AuthenticationBackend)` constructed with `secret_key=settings.SECRET_KEY` — the constructor takes only `secret_key` (G2 correction: sqladmin 0.20 has no `secret_url` parameter; the login URL is its stock `admin:login` route) and mounts its own `SessionMiddleware` with that key; the app does **not** add one manually. `login` reads the standard form (phone + password), verifies via `AuthService` (incl. `is_active`), requires `role == "admin"`; stores `{"user_id": ...}` in the sqladmin session. `logout` clears it. `authenticate` checks the stored key. The stock sqladmin login page is used, no custom templates.
- `UserAdmin` form (G2 correction — sqladmin 0.20 canonical pattern): `form_overrides = {User.password_hash: PasswordField}` renders the column as a write-only `PasswordField("password")` (never populated from the model, never rendered back); an `on_model_change` override applies the semantics — on **create**: required → `validate_password` → `hash` → store; on **edit**: blank leaves `password_hash` unchanged, filled → validate + hash. Help text = `PASSWORD_POLICY_HINT_RU`. The lock fields (`failed_login_attempts`, `lock_level`, `locked_until`) render in the stock form — **the admin lockout reset is editing the user and clearing them** (zero / zero / null).
...
`backend/src/seed/seed.py` gains a `seed_staff_users()` step (skipped when `ENV == "production"`): admin `+79990000001 / admin12345` and master `+79990000002 / master12345` linked to the first seeded staff card. Passwords satisfy the policy; the obviously-fake values are deliberate (public repo, dev/demo only). **Deployment note:** staging environments must set `ENV=production` (or otherwise never run the seed) — a staging box running with a dev ENV would accept these demo credentials.
...
Outside the `(main)` route group (no sidebar/user block). Controlled inputs (phone, password), submit → `login()` → success: `router.replace(returnTo ?? "/")`; failure: inline error + `showToast` (existing `parseApiError` + toast system, repo style). Already authenticated (mount-time `/me` says so) → redirect to `/`. No policy hint on this page (deliberate, decision 10).
...
- The existing bottom-left user block (`Menubar.tsx` «User Avatar», currently hardcoded «А»/«Админ») shows the real logged-in user: avatar (photo from the linked staff card's `avatar_url` when set, else the initial of the displayed name) + staff card name when linked, **else «Аноним»** (#262 amendment 2026-09-10, user decision: no phone, no role label on the block). It hosts the logout control: a **text button «Выйти»** beside the block (#262 pin 2026-09-10: text button only, do not build a menu — the user popup menu arrives in #262 and replaces it).
...
| Locked out | 429 | `AUTH_LOCKED_OUT` | timed locks: + `Retry-After`; hard lock: no header, message points to admin reset; correct password is also rejected |
...
2. **Master signs in — same interface (G2 user decision)** — the master sees the same sections and controls as the admin (display tuning per role is a future task); a forbidden write (e.g. creating a payment) is rejected by the API with 403 and surfaces as the standard error toast. → e2e: login as seeded master, assert identical nav; attempt a forbidden write → visible error.
...
- **API (pytest `TestClient`):** login success / wrong password; the lockout ladder — 3rd failure → 429 + `Retry-After` (15-min lock, correct password also rejected), after expiry 3 more → 1-hour lock, 3 more → hard lock (no `Retry-After`, admin-reset message, survives correct password); clearing the lock fields (the admin reset) restores login; a successful login resets the ladder; per-IP counter trips at 20 across two phones; `me` with/without cookie; logout revokes; login rotates a presented stale session; per-router guard matrix parametrized by role × representative endpoints; user-settings own-only scoping (session-user reads own settings OK; foreign/legacy `user_id` param rejected); public allowlist (anonymous dictionary GETs 200, anonymous `POST /records` 201); Sec-Fetch-Site rejection (`cross-site` → 403 on an authenticated mutation; missing header passes); sqladmin auth on a dedicated app fixture.
...
- Per-role display tuning in the admin UI (hiding sections, read-only rendering of «Платежи»/«Клиенты», hiding «Материалы» for masters) — **deferred by user decision (G2)**: v1 ships an identical interface for both roles; the backend 403 enforces permissions meanwhile.
...
- **Deliberate posture deviations recorded (G1b/G2):** password floor 8 (vs NIST 15 single-factor); SameSite=Lax + Sec-Fetch-Site (vs strictest OWASP menu); idle 7d/cap 30d (vs OWASP 15–30 min idle); in-memory per-IP counter (single-process runtime; the per-phone ladder is DB-backed). Each is a conscious MVP tradeoff for an internal staff tool, revisit at production hardening.
