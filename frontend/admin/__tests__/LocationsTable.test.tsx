...
describe('LocationsTable', () => {
  ...
  // clamps q to ≥2 chars and sends it to getLocations. Rows render exactly
  ...
  it('searches server-side via ?q= (fetch carries q, rows stay server-returned)', async () => {
    ...
    await waitFor(() => {
      expect(mockGetLocations).toHaveBeenCalledWith(expect.objectContaining({ q: 'Невском' }));
    });
  ...
  it('does not fire q on 1 char (≥2 clamp — treated as unset)', async () => {
    ...
    const searchInput = screen.getByLabelText('Поиск по названию или адресу');
    fireEvent.change(searchInput, { target: { value: 'Н' } });

    // Raw input state reflects the char, but no q fetch fires
    await waitFor(() => {
      expect(searchInput).toHaveValue('Н');
    });
    expect(mockGetLocations).toHaveBeenCalledTimes(1);
    expect(mockGetLocations.mock.calls[0][0]).not.toHaveProperty('q');
  });
