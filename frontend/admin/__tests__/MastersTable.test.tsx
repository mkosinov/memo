...
describe('MastersTable', () => {
  ...
  // clamps q to ≥2 chars and sends it to getMasters. Rows render exactly
  ...
  it('searches server-side via ?q= (fetch carries q, rows stay server-returned)', async () => {
    ...
    await waitFor(() => {
      expect(mockGetMasters).toHaveBeenCalledWith(expect.objectContaining({ q: 'Ольга' }));
    });
  ...
  it('does not fire q on 1 char (≥2 clamp — treated as unset)', async () => {
    ...
    await waitFor(() => {
      expect(searchInput).toHaveValue('О');
    });
    expect(mockGetMasters).toHaveBeenCalledTimes(1);
    expect(mockGetMasters.mock.calls[0][0]).not.toHaveProperty('q');
  });
